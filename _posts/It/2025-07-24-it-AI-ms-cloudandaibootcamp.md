...
안녕하세요. 저도 처음에는 애저 AI 파운드리가 어렵고 복잡하게 느껴졌습니다. 웹 개발자 입장에서 이 파운드리가 어떤 배경을 갖고 있고 어떤 소프트웨어와 연결되는지 살펴봤습니다.
...
전사, 쇼 노트 작성, 콘텐츠 연결 등을 묶어 팟캐스트 팩토리처럼 운영할 수 있게 된 셈입니다. 여러분도 자신이 가진 문제를 해결할 AI 팩토리를 만들어 보시길 바랍니다.

이제 발표를 프리앙카에게 넘기겠습니다.

다음 데모는 마틴 님이 진행하며, GitHub Copilot 활용 방법을 소개합니다.

Copilot은 Visual Studio Code에서 탭 키로 코드 제안을 받거나, 질문을 던져 기능 설명을 받을 수 있습니다. 예를 들어 웹사이트의 아키텍처를 설명해달라고 요청하면 관련 정보를 분석해줍니다. 만다린어로 질문하면 만다린어로 답하는 식으로 여러 언어도 지원합니다.

이제 최신 기능을 살펴보겠습니다. Copilot의 에이전트 모드를 켜면 코드베이스를 분석하고 변경을 제안하거나 직접 수행합니다. 예를 들어 연사 안내 페이지에서 USB 헤드셋 정보가 빠져 있다면, Copilot이 해당 파일을 찾아 내용을 추가합니다. 전체 코드베이스를 대상으로 작업하고 CSS 스타일링까지 적용해줍니다.

또한, GitHub에서 이슈가 발생했을 때 이를 Copilot에게 할당하면, Pull Request를 생성하고 변경 사항을 적용해줍니다. 작업 진행 상황도 실시간으로 확인할 수 있습니다.

Copilot은 코드 리뷰도 할 수 있습니다. 코드 내 타이포나 버그를 감지하고 주석으로 안내합니다. 데브옵스 환경에서 특히 유용하며, 기존 개발 환경에도 쉽게 도입할 수 있습니다.

Copilot은 자동화 기능을 제공하지만, 사람의 판단과 감독은 여전히 필요합니다. 보안, 품질, 윤리적 사용을 위한 프로세스도 함께 고려해야 합니다.
...
발표자는 관련 리소스를 채팅창에 공유하고, 이후 세션은 웹사이트에서 확인할 수 있다고 안내했다.
</details>

## 세션 1: 스캇 핸셀맨 – Azure AI Foundry 데모
...
CLI, GitHub, VS Code 등 다양한 환경 지원.
 * 사례 데모: 팟캐스트 웹사이트 개선
   * 연사 안내 페이지 업데이트.
   * 에이전트가 파일 찾고 내용 추가, 스타일 수정.
   *  자동 Pull Request 생성 및 배포.

## 세션 3: Copilot으로 게임 개발 + 배포
...
## 핵심 메시지
에이전트와 Copilot은 반복 작업의 자동화 범위를 넓힌다. 다만 데모 결과를 그대로 일반화할 수는 없다. 생성된 코드와 배포 설정은 사람이 검토하고, 프롬프트에는 작업 범위와 검증 조건을 명시해야 한다.
