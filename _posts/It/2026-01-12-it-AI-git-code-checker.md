...
### Git Hook의 장점
이 조건에서는 **로컬 Git Hook**이 꽤 괜찮은 대안이었다. 저장소 권한이나 팀 승인 없이 개인 로컬 환경에서만 작동하고, 개인 API 키만 있으면 인증이 끝난다. 설정 변경 없이 바로 적용할 수 있고, 필요에 맞게 스크립트를 직접 고칠 수 있다는 점도 좋았다.
...
API 키는 Google AI Studio(https://aistudio.google.com/app/apikey)에서 발급받을 수 있다. (Gemini CLI는 `gemini` 실행 시 Google 로그인으로 인증할 수도 있다.)
...
application.yaml, application.yml, *.properties 파일은:
  ...
  review:
    ...
    steps:
      ...
      - name: Setup Gemini CLI
        run: |
          npm install -g @google/gemini-cli
          # API 키는 환경변수로 전달 (이후 스텝까지 유지되도록 $GITHUB_ENV 에 기록)
          echo "GEMINI_API_KEY=${{ secrets.GEMINI_API_KEY }}" >> $GITHUB_ENV
      
      - name: Run Code Review
        run: |
          # PR의 변경 파일 분석 (diff를 stdin으로 전달, -p로 지시문 전달)
          git diff origin/main...HEAD | gemini -p "코드 리뷰"
~~~

# 9. 환경이 갖춰지면 해보고 싶은 것들

지금의 Git Hook 방식은 "내가 통제할 수 있는 환경"이라는 제약 안에서 고른 선택이다. 권한이나 팀 합의가 갖춰진다면 더 나은 방법으로 옮겨갈 여지가 있어, 단계별로 정리해 둔다.

## 9.1. 팀 표준화: Pre-commit Framework

지금처럼 쉘 스크립트를 직접 관리하는 대신 [Pre-commit Framework](https://pre-commit.com/)를 쓰면, Hook 구성을 설정 파일(`.pre-commit-config.yaml`)로 버전 관리하고 팀원과 공유할 수 있다. `pre-commit install` 한 번이면 팀원 환경에 동일하게 적용되고, `SKIP=gemini-review git commit ...`처럼 특정 Hook만 건너뛰는 것도 표준화된 방식으로 가능하다. 개인 스크립트를 팀 표준으로 끌어올리는 가장 현실적인 다음 단계라고 본다.
...
## 9.2. 서버 통합: Bamboo + Bitbucket Code Insights

저장소 관리자 권한과 보안팀 승인이 확보된다면, 로컬 Hook 대신 Bamboo Build Plan에 AI 리뷰 단계를 추가하고 결과를 Bitbucket Code Insights로 PR에 코멘트하는 방식이 더 낫다. 개인 환경에 의존하지 않고 팀 전체에 강제 적용되며, PR 단계에서 통과/실패를 브랜치 보호 규칙과 엮을 수 있다는 점이 장점이다. 다만 이 방식은 처음에 제약사항으로 막혔던 부분(관리자 권한, 외부 API 호출 승인)이 모두 해결되어야 가능하다.

```
Developer → Commit → Bitbucket → Webhook → Bamboo Build
                                              ↓
                                    AI Review Stage → Code Insights → PR Comment
```
...
3. **서버 통합(권한 확보 시)**: Bamboo 단계 추가 + Bitbucket PR 코멘트

각 단계의 비용·효과를 단정적으로 수치화하긴 어렵지만, 적어도 "권한 없이 당장 시작" → "팀 공유" → "서버 강제 적용" 순으로 점진적으로 올리는 흐름이 가장 무리가 없었다.

# 10. 마치며

저장소 권한 부족, 복잡한 인증, 팀 프로세스 제약이라는 상황에서, 결국 "내가 통제할 수 있는" 로컬 Git Hook으로 AI 코드 리뷰를 붙였다. 거창한 인프라 없이도 커밋 전에 코드 품질을 한 번 점검하고, 치명적인 문제는 커밋을 막고, 커밋 메시지까지 자동으로 채워주는 수준은 만들 수 있었다.

다만 쓰면서 분명해진 점은, AI 리뷰는 어디까지나 참고용이라는 것이다. 실제로 YAML/Properties 같은 파일은 오판이 잦아 아예 `[BLOCK]`을 금지하는 예외를 둬야 했고, 최종 판단은 결국 사람이 해야 했다. 환경이 갖춰지면 9절에 적은 것처럼 Pre-commit Framework나 Bamboo 통합으로 옮겨갈 생각이지만, 권한이 없는 지금 상황에서는 이 방식이 가장 현실적인 타협점이었다.
