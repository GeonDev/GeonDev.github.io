...
* **동시 주문/재고 차감 판단 기준** : 충돌이 많고 반드시 한 번에 하나씩 처리해야 한다면 DB의 비관적 락(`SELECT ... FOR UPDATE`)이나 원자적 조건 업데이트(`UPDATE stock SET qty = qty - 1 WHERE qty > 0`)가 단순하고 안전하다. 충돌이 적으면 버전 컬럼 기반 낙관적 락으로 재시도한다. Redis 분산 락도 사용할 수 있지만 락 만료, 락 소유자 검증, 네트워크 장애를 함께 고려해야 하므로 강한 정합성이 필요하면 DB 락을 우선 검토한다.
...
* **Oracle SQL 튜닝 포인트** : Oracle에서도 실행계획 확인, 복합 인덱스 선두 컬럼, 카디널리티, 조인 순서, 정렬 비용을 먼저 본다.  
  날짜 컬럼에 `TRUNC(date_col)`처럼 함수를 걸면 일반 인덱스를 타기 어려울 수 있으므로 `date_col >= :from AND date_col < :to` 같은 범위 조건으로 바꾸거나 함수 기반 인덱스를 검토한다. 대량 정산·환급 조회는 온라인 조회와 배치 집계를 분리하고, 일자·상태·가맹점/환급창구 ID 조합으로 인덱스를 설계한다.
...
* **결제·환급 상태 머신(State Machine)** : 결제, 환급, 세금환급(TRS)처럼 돈과 외부 연동이 얽힌 업무는 `성공/실패`만으로 상태를 표현하면 복구가 어렵다.  
  `REQUESTED`, `PENDING`, `APPROVED`, `REFUND_PENDING`, `REFUNDED`, `FAILED`, `UNKNOWN`처럼 업무 단계와 결과 미확인 상태를 나누고, 현재 상태에서 허용되는 전이만 적용해야 한다. 외부 API timeout은 실패가 아니라 결과를 모르는 상태일 수 있으므로 조회, webhook, 정기 대사로 최종 상태를 맞춘다.


* **TRS(Tax Refund Service, 외국인 세금환급 서비스)** : 외국인 관광객이 국내에서 구매한 물품의 부가세 등을 환급받는 업무 시스템이다.  
  가맹점, 환급창구, 공항/세관, 결제/환급 기관 등 외부 연동이 많고, 환급 요청 ID, 외부 거래번호, 금액, 상태, 처리 시각을 기준으로 정합성을 맞춘다. 중복 환급, 늦게 도착한 취소 이벤트, 외부 승인 성공 후 로컬 DB 반영 실패 같은 예외를 전제로 설계해야 한다.


* **정산·환급 대사(Reconciliation)** : 로컬 DB와 외부 결제/환급/기관 시스템의 거래 내역을 비교해 불일치를 찾고 보정하는 작업이다.  
  실시간 처리에서 누락이나 timeout이 발생할 수 있으므로 주문 ID, 결제 ID, 환급 요청 ID, 승인번호, 금액, 상태, 처리일자를 기준으로 주기적으로 대사한다. 불일치는 자동 재조회·재처리하거나 운영 검토 상태로 남기며, 대사 작업 자체도 중복 실행에 안전해야 한다.
...
* **상품 상세(Item View) API** : 사용자가 상품 정보를 탐색하고 구매를 결정하는 화면에 필요한 데이터를 제공하는 API다.  
  상품 기본 정보, 이미지, 옵션/SKU, 가격, 재고, 프로모션, 배송 정보, 리뷰 요약처럼 변경 빈도와 정합성 요구가 다른 데이터를 조합한다. 상세 조회는 빠르게 응답해야 하지만, 가격·재고는 주문 확정 시점에 서버에서 다시 검증해야 한다.


* **SKU(Stock Keeping Unit)와 상품 옵션** : 상품은 하나의 대표 상품(Item) 아래에 색상, 사이즈, 구성 같은 옵션 조합을 가질 수 있고, 실제 재고·가격은 옵션 조합인 SKU 단위로 관리되는 경우가 많다.  
  예를 들어 "티셔츠"는 상품이고 "검정/M"은 SKU다. 화면에는 옵션별 구매 가능 여부를 보여주되, 주문 시에는 선택한 SKU의 판매 가능 상태와 재고를 최종 확인한다.


* **가격·재고 정합성** : 상품 상세 화면에 보이는 가격과 재고는 캐시나 조회 모델 때문에 일시적으로 오래된 값일 수 있다.  
  그러나 주문 생성·결제 직전에는 서버가 상품, 옵션, 회원 등급, 쿠폰, 프로모션, 재고를 기준으로 최종 가격과 구매 가능 여부를 다시 계산해야 한다. 프론트엔드가 보낸 가격이나 재고 값을 신뢰하면 안 된다.
