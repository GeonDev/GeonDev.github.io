...
* **인덱스(Index) 유형**
  ...
  * **Unique Index** : Unique Index로 컬럼이 잡혀 있으면 해당 컬럼에는 중복된 데이터가 들어가지 않는다. 단,  
    `NULL` 허용 방식은 DBMS마다 다르다. MySQL과 PostgreSQL은 기본적으로 여러 `NULL`을 허용하지만, SQL Server의 일반 Unique Index는 하나의 `NULL`만 허용한다. Oracle은 모든 키 컬럼이 `NULL`인 행을 B-Tree 인덱스에 저장하지 않는 등 구현 차이가 있다. Primary Key는 DBMS와 관계없이 `NULL`을 허용하지 않는다.
...
* **소프트웨어 아키텍처** : 시스템의 주요 구성 요소, 책임, 의존관계와 변경 원칙을 정하는 구조적 결정이다. 좋은 아키텍처는 특정 패턴을 많이 적용한 구조가 아니라, 중요한 변경을 국소화하고 품질 요구사항을 만족하면서 운영 가능한 구조다.
...
* **Architecture Decision Record(ADR)** : 중요한 아키텍처 결정을 배경, 선택지, 결정, 결과와 함께 짧게 기록하는 문서다. “Kafka를 사용한다”보다 해결하려는 문제, 검토한 대안, 감수한 단점과 재검토 조건을 남겨야 이후 변경 근거를 이해할 수 있다.
...
* **일관성 모델**
  ...
  * **인과적 일관성(Causal Consistency)** : 원인과 결과 관계가 있는 작업의 순서를 보장하는 모델이다. 모든 작업의 전역 순서보다 약하지만 연관된 변경의 관찰 순서를 유지한다.


### MSA에서 정합성을 유지하는 방법

* **정합성의 기본 원칙** : MSA에서 모든 데이터를 하나의 분산 트랜잭션으로 묶는 것이 정답은 아니다. 반드시 동시에 확정돼야 하는 규칙은 한 서비스와 로컬 트랜잭션 안에 두고, 서비스 경계를 넘는 상태 변화는 이벤트, 멱등성, 보상과 대사 작업을 조합해 수렴시킨다.


* **1. 정합성 경계를 서비스 안으로 모은다** : 하나의 업무 불변식(Invariant)을 지키는 데이터는 가능하면 같은 서비스가 소유하고 하나의 DB 트랜잭션으로 변경한다. 예를 들어 재고 수량이 음수가 되면 안 된다는 규칙은 재고 서비스 내부에서 조건부 UPDATE나 락으로 원자적으로 보장한다. 서비스마다 데이터를 나눴다는 이유로 하나의 규칙까지 여러 서비스에 분산하면 정합성 유지가 훨씬 어려워진다.


* **2. 데이터의 단일 소유자를 정한다** : 주문 상태는 주문 서비스, 결제 승인 상태는 결제 서비스처럼 각 데이터의 최종 판단 주체를 하나로 둔다. 다른 서비스는 소유 DB를 직접 수정하지 않고 API나 이벤트를 통해 변경을 요청한다. 여러 서비스가 같은 값을 각각 원본으로 관리하면 어느 값이 맞는지 결정할 수 없다.


* **3. 상태 변경과 이벤트 기록을 원자적으로 처리한다** : 업무 데이터를 커밋한 뒤 별도로 메시지를 발행하면 둘 중 하나만 성공하는 Dual Write 문제가 생긴다. Transactional Outbox를 사용해 업무 변경과 발행할 이벤트를 같은 로컬 트랜잭션에 저장하고, 별도 릴레이가 이벤트를 브로커로 전달하게 한다.


* **4. 메시지 중복을 전제로 소비한다** : 메시지 브로커와 Outbox 릴레이는 장애 복구 과정에서 같은 이벤트를 다시 전달할 수 있다. 소비자는 event ID를 Inbox나 처리 이력에 기록하고, 업무 변경과 중복 처리 기록을 같은 로컬 트랜잭션으로 커밋한다. 단순 조회 후 INSERT보다 Unique Key, 조건부 UPDATE, upsert 같은 DB 제약을 함께 사용해야 동시 중복 처리도 막을 수 있다.


* **5. 여러 서비스의 업무 흐름은 Saga로 관리한다** : 주문·결제·재고처럼 여러 로컬 트랜잭션이 순서대로 실행되는 작업은 Saga 상태를 명시하고 각 단계의 성공·실패·보상 이벤트를 관리한다. 중앙 오케스트레이터를 사용하면 전체 흐름과 timeout을 파악하기 쉽고, Choreography를 사용하면 중앙 의존성은 줄지만 이벤트 흐름 추적이 어려워질 수 있다.


* **6. 순서 역전과 동시 수정을 처리한다** : 같은 Aggregate의 이벤트는 동일한 파티션 키를 사용해 순서를 유지하고, 이벤트에 aggregate ID와 version을 포함한다. 소비자는 현재 version보다 오래된 이벤트를 무시하거나 보류하며, HTTP 기반 수정은 version 컬럼·ETag·조건부 UPDATE로 Lost Update를 방지한다. 네트워크 전체에서 완벽한 전역 순서를 만들기보다 업무상 순서가 필요한 단위를 명확히 정한다.


* **7. 중간 상태를 숨기지 않고 모델링한다** : 최종적 일관성을 사용하면 `주문 생성 완료` 직후 결제나 재고가 아직 확정되지 않을 수 있다. 이를 오류처럼 숨기지 말고 `PAYMENT_PENDING`, `STOCK_RESERVING`, `CONFIRMED`, `FAILED` 같은 상태로 표현한다. API는 처리 중 상태와 재조회 방법을 제공하고, timeout 이후에도 백그라운드 처리가 완료될 수 있음을 고려한다.


* **8. 대사와 수동 복구를 마지막 안전망으로 둔다** : 네트워크 장애, 운영자 개입, 외부 결제 시스템 오류까지 모든 경우를 실시간 로직만으로 해결하기는 어렵다. 주기적으로 주문 금액과 결제 승인, 예약 재고를 비교하는 대사 작업을 실행하고, 불일치를 재처리하거나 운영자가 감사 로그를 보고 보정할 수 있게 한다.
...
* **상황별 선택 기준**
  ...
  | 상황 | 우선 검토할 방법 | 주의점 |
  |------|------------------|--------|
  | 한 서비스·한 DB 안의 변경 | 로컬 트랜잭션, DB 제약, 조건부 UPDATE | 불필요하게 이벤트로 분리하지 않는다. |
  | DB 변경 후 이벤트 발행 | Transactional Outbox | 중복 발행 가능성이 있어 소비자가 멱등해야 한다. |
  | 여러 서비스의 단계적 업무 처리 | Saga와 보상 트랜잭션 | 중간 상태, timeout, 보상 실패를 모델링한다. |
  | 자원의 임시 확보가 필요한 처리 | TCC 또는 예약 상태 | 만료와 Confirm·Cancel 재시도를 설계한다. |
  | 중복 메시지 처리 | Inbox, event ID, Unique Key, upsert | 조회 후 저장만으로는 동시 실행을 막기 어렵다. |
  | 순서가 중요한 상태 변경 | Aggregate별 key와 version | 모든 이벤트의 전역 순서를 요구하지 않는다. |
  | 시스템 간 장기 불일치 탐지 | 대사 작업과 수동 복구 도구 | 재처리 이력과 감사 로그를 남긴다. |


* **주문·결제·재고 예시**
  1. 주문 서비스가 주문을 `PENDING`으로 저장하면서 Outbox에 `OrderCreated` 이벤트를 같은 트랜잭션으로 기록한다.
  2. 재고 서비스는 이벤트 ID 중복을 확인하고 재고를 조건부로 예약한 뒤 `StockReserved` 또는 `StockReservationFailed`를 발행한다.
  3. 결제 서비스는 재고 예약 성공 후 결제를 승인하고 `PaymentApproved` 또는 `PaymentFailed`를 발행한다.
  4. 주문 서비스는 두 결과가 성공하면 `CONFIRMED`로 변경한다. 결제가 실패하면 재고 예약 취소를 요청하고 주문을 `FAILED`로 변경한다.
  5. 이벤트가 중복되거나 순서가 늦어져도 각 서비스는 event ID와 업무 상태 전이 규칙으로 같은 부수 효과가 반복되지 않게 한다.
  6. 일정 시간 이상 `PENDING`인 주문은 timeout 처리하고, 대사 작업이 주문·결제·재고 상태를 비교해 누락된 이벤트를 재처리하거나 운영자에게 알린다.
...
* **분산 트랜잭션과 데이터 정합성**
  ...
  * **TCC(Try-Confirm-Cancel)** : 각 참여 서비스가 자원을 임시 확보(Try)한 뒤 모두 성공하면 확정(Confirm), 실패하면 취소(Cancel)하는 방식이다. 예약·재고 확보처럼 명시적인 보류 상태를 만들 수 있는 업무에 적합하지만 각 단계의 API와 만료·복구 로직을 별도로 구현해야 한다.
...
* **CQRS (Command Query Responsibility Segregation)** : 상태를 변경하는 명령 모델과 조회 모델을 분리하는 패턴이다. 같은 DB에서 코드 모델만 나눌 수도 있고, 필요하면 별도 읽기 저장소와 비정규화된 뷰를 둘 수도 있다. 읽기·쓰기 요구가 크게 다를 때 유용하지만 동기화 지연과 운영 대상이 증가하므로 단순 CRUD에는 과할 수 있다.


* **Event Sourcing** : 현재 상태만 저장하지 않고 상태를 변경시킨 이벤트를 순서대로 저장하고 재생해 상태를 복원하는 방식이다. 감사와 시점 복원에 유리하지만 이벤트 불변성, 스키마 진화, 재생 비용과 외부 부수 효과를 다루기 어렵다. CQRS와 함께 사용할 수 있지만 두 패턴이 서로 필수인 것은 아니다.
...
* **Inbox 패턴** : 소비자가 수신한 메시지 ID와 처리 상태를 자신의 DB에 기록해 중복 처리를 막는 방식이다. 업무 변경과 Inbox 기록을 같은 로컬 트랜잭션으로 처리하고, 실패 메시지는 재시도·DLQ·수동 복구 정책에 따라 관리한다.
...
* **Load Shedding(부하 차단)** : 시스템 용량을 넘는 요청을 조기에 거부해 이미 처리 중인 요청과 핵심 기능을 보호하는 방식이다. 우선순위, Rate Limit, 큐 상한과 `429 Too Many Requests` 또는 `503 Service Unavailable` 응답 및 재시도 힌트를 함께 설계한다.


* **Backpressure(배압)** : 생산 속도가 소비 속도보다 빠를 때 소비자가 처리 가능한 양만 받거나 생산 속도를 늦추게 하는 흐름 제어다. 메시지 소비 동시성, prefetch, bounded queue, Reactive Streams의 demand 등을 이용해 메모리와 지연의 무제한 증가를 막는다.


* **복구와 검증** : 장애가 끝났다고 데이터가 자동으로 정상화되는 것은 아니다. 재처리·대사·수동 보정 절차와 Runbook을 준비하고, Chaos Engineering이나 장애 주입 테스트로 timeout, 격리, 자동 복구와 알림이 실제로 동작하는지 제한된 환경부터 검증한다.

## 멀티테넌시

* **테넌트(Tenant)** : 하나의 애플리케이션을 독립적으로 사용하는 고객 조직이나 계정을 뜻한다. SaaS 서비스에서는 회사, 학교, 병원, 쇼핑몰 입점사처럼 데이터·사용자·권한·설정을 다른 고객과 분리해야 하는 단위가 테넌트가 된다.
  * 예를 들어 협업 SaaS를 A회사와 B회사가 함께 사용한다면 A회사와 B회사가 각각 하나의 테넌트다.
  * 한 테넌트에는 여러 사용자가 속할 수 있으며, 같은 사용자가 여러 테넌트에 소속될 수도 있다.
  * 테넌트는 반드시 회사만 의미하지 않는다. 서비스 정책에 따라 개인 계정, 프로젝트, 워크스페이스나 판매자가 격리 단위가 될 수 있다.
  * 시스템은 현재 요청이 어느 테넌트에 속하는지 식별하고, 해당 테넌트가 소유한 데이터와 기능에만 접근하도록 제한해야 한다.
...
* **멀티테넌시와 멀티테넌트의 차이**
  * **멀티테넌시(Multi-tenancy)** : 여러 테넌트를 지원하는 아키텍처나 운영 **방식**을 뜻하는 명사다. 예: `멀티테넌시 아키텍처`, `멀티테넌시 전략`
  * **멀티테넌트(Multi-tenant)** : 여러 테넌트가 사용하는 시스템의 **특성**을 나타낸다. 예: `멀티테넌트 애플리케이션`, `멀티테넌트 환경`
  * 이 문서에서는 설계 방식과 선택 기준에는 `멀티테넌시`, 시스템이나 환경의 특성을 설명할 때는 `멀티테넌트`를 사용한다.


* **테넌트 컨텍스트(Tenant Context)** : 현재 요청이나 작업이 어느 테넌트에 속하는지를 나타내는 정보다. 로그인 토큰·세션처럼 서버가 검증한 인증 정보에서 테넌트 ID를 확정하고 서비스, Repository, 비동기 작업과 메시지까지 전달한다. 사용자가 임의로 보낸 요청값만으로 테넌트를 결정하면 안 된다.


* **테넌트 격리 모델**
  * **공유 DB·공유 스키마** : 모든 테넌트가 같은 DB와 테이블을 사용하고, 각 행에 `tenant_id`를 저장해 소유 테넌트를 구분하는 **행 단위 격리 방식**이다. 비용과 운영 부담은 낮지만 모든 조회·수정·삭제와 Unique Key에 테넌트 범위를 포함하고 교차 접근을 구조적으로 차단해야 한다.
  * **공유 DB·테넌트별 스키마** : DB 인스턴스는 공유하고 스키마를 분리한다. 데이터 경계와 테넌트별 마이그레이션은 명확하지만 스키마 수가 많아지면 배포와 커넥션 관리가 복잡해진다.
  * **테넌트별 DB** : 데이터베이스를 완전히 분리해 격리, 백업·복구와 규제 대응에 유리하다. 대신 인프라 비용과 연결, 마이그레이션, 통합 조회의 운영 부담이 가장 크다.
  * **혼합 모델(Hybrid)** : 일반 테넌트는 공유하고 대규모·규제 대상 테넌트만 전용 DB로 분리한다. 테넌트를 다른 저장소로 이동할 수 있는 라우팅과 마이그레이션 절차가 필요하다.


* **공유 DB·공유 스키마 예시** : 고객사 A와 B의 주문을 별도 테이블로 만들지 않고 하나의 `orders` 테이블에 함께 저장한다. `tenant_id = 100`은 고객사 A, `tenant_id = 200`은 고객사 B처럼 각 행의 소유자를 표시한다.
  ...
  `order_no`는 테넌트마다 같은 값을 사용할 수 있으므로 `(tenant_id, order_no)`를 하나의 Unique Key로 묶는다. 조회뿐 아니라 `UPDATE`, `DELETE`에도 `tenant_id` 조건이 필요하며, 자주 사용하는 조회 조건에 맞춰 `(tenant_id, ...)` 복합 인덱스를 설계한다.


* **테넌트 ID 적용 시 주의점** : 클라이언트가 요청 파라미터나 헤더로 보낸 `tenant_id`를 그대로 신뢰하면 다른 테넌트 ID로 바꿔 호출할 수 있다. 로그인 토큰·세션처럼 서버가 검증한 인증 정보에서 테넌트 ID를 확정하고 Repository까지 전달해야 한다. 공통 쿼리 필터나 PostgreSQL Row-Level Security 같은 DB 기능을 보조 방어선으로 사용할 수 있지만, 배치·Native Query·관리자 기능까지 누락 없이 적용되는지 테스트해야 한다.


* **테넌트 격리의 적용 범위** : DB뿐 아니라 캐시 키, 파일 경로, 메시지, 검색 인덱스, 로그, 메트릭과 백업에도 테넌트 식별자와 접근 정책을 적용한다. 관리자 기능과 배치 작업도 테넌트 범위를 명시하지 않으면 대량 교차 노출 경로가 될 수 있다.


* **Noisy Neighbor 문제** : 한 테넌트의 과도한 트래픽이나 대용량 쿼리가 공유 CPU, 메모리, 커넥션 풀을 점유해 다른 테넌트의 품질을 떨어뜨리는 현상이다. 테넌트별 Rate Limit, 동시 실행 제한, 큐, 쿼터와 사용량 측정으로 공정성을 유지한다.


* **테넌트별 설정과 기능** : 요금제, 기능 플래그, 로케일, 데이터 보존 기간처럼 테넌트별로 달라지는 설정은 코드 분기보다 명시적인 설정 모델로 관리한다. 설정 변경의 권한, 감사 로그, 캐시 무효화와 기본값을 정의한다.


* **마이그레이션과 운영** : 스키마가 여러 개거나 테넌트별 DB를 사용하면 일부 테넌트만 마이그레이션에 실패할 수 있다. 적용 버전을 테넌트별로 추적하고 재시도·롤백·격리 절차를 마련하며, 백업 복구도 특정 테넌트 단위로 가능한지 확인한다.


* **멀티테넌시 아키텍처 선택 기준** : 비용만 비교하지 않고 데이터 민감도, 규제, 테넌트 수와 크기 편차, 커스터마이징 요구, 장애 영향 범위와 복구 단위를 함께 고려한다. 초기에는 공유 모델로 시작하더라도 대형 테넌트를 분리할 가능성이 있다면 데이터 이동과 라우팅 경계를 미리 명확히 한다.
