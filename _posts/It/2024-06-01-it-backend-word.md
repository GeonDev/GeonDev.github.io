...
>
> 참고: [backend-interview-question](https://github.com/ksundong/backend-interview-question?tab=readme-ov-file)

업무만 하다 보니 이론을 너무 까먹어서 정리하려고 한다.

단순 정의보다 "왜 필요한가", "어떤 문제가 생기는가", "실무에서는 어떻게 선택하는가"까지 함께 이해하는 것이 중요하다. 그래서 각 용어는 정의와 함께 주의점, 선택 기준, 함께 떠올릴 만한 질문까지 같이 정리한다.


# 용어정리 – 데이터베이스

* **트랜잭션** : 하나의 논리적 기능을 수행하기 위한 작업 단위, 쪼갤 수 없는 업무의 최소 단위. 모든 작업이 성공하면 커밋(Commit)하고, 중간에 실패하면 롤백(Rollback)하여 이전 상태로 되돌린다.
...
* **JWT의 동작 방식**
  ...
  * **인가(Authorization)** : JWT에는 사용자의 권한 정보나 추가적인 메타데이터가 포함될 수 있어, 서버는 JWT를 통해 사용자의 권한을 확인할 수 있다.
...
* **@Transactional 특징**
  ...
  * Spring의 기본 트랜잭션 AOP는 Proxy Mode이므로 `private 메서드`, `final 메서드`, 같은 클래스 내부에서 직접 호출하는 self-invocation에는 적용되지 않는다.
...
HTTP 프로토콜을 기반으로 하므로 기존 웹 기술과 쉽게 통합할 수 있고 클라이언트/서버를 분리 개발하여 확장성이 좋다. 다만 HTTP 헤더와 같은 추가 메타데이터로 네트워크 오버헤드가 발생할 수 있고, REST는 표준이 아니라 아키텍처 스타일이므로 구현 방식에 따라 차이가 있다.
...
* **Redis (Remote Dictionary Server)** : 인메모리(In-Memory) 기반의 Key-Value 저장소. 모든 데이터를 메모리에 저장해 매우 빠르며, 캐시뿐 아니라 세션 저장소, 분산 락, 메시지 큐, 순위표(Ranking) 등 다양하게 활용된다. Redis 명령 실행은 기본적으로 단일 스레드 이벤트 루프에서 처리되어 개별 명령의 원자성을 보장한다. 다만 네트워크 I/O, 백그라운드 작업, 일부 기능은 별도 스레드를 사용할 수 있으므로 "Redis 전체가 항상 완전한 싱글 스레드"라고 단순화하면 안 된다. 무거운 명령은 전체 처리를 블로킹시킬 수 있어 주의해야 한다. (예: `KEYS *`, 큰 범위의 `SORT`)
