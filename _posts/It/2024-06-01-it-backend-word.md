...
* **JOIN 기법 종류**
  ...
  * **병합 조인 (Sort Merge Join)** : 두 Join 입력이 작지 않지만 Join 열을 (인덱스로부터 미리) 정렬된 상태로 가져올 수 있는 경우 가장 빠른 방법, 두 입력의 크기가 서로 비슷할 경우에는 Merge Join과 Hash Join 성능이 비슷하지만, 두 입력의 크기가 서로 많이 다를 경우 Hash Join 성능이 더 좋다. 정렬된 양쪽 결과를 스캔하는 방식이므로 인덱스 유무는 중요하지 않다.
  * **해시 조인 (Hash Match Join)** : Join 입력의 크기가 크고 정렬되지 않았으며 인덱싱되지 않은 입력을 효율적으로 처리하는 방법
