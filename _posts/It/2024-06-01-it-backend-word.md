...
* **Oracle SQL 튜닝 포인트** : Oracle에서도 실행계획 확인, 복합 인덱스 선두 컬럼, 카디널리티, 조인 순서, 정렬 비용을 먼저 본다.
  날짜 컬럼에 `TRUNC(date_col)`처럼 함수를 걸면 일반 인덱스를 타기 어려울 수 있으므로 `date_col >= :from AND date_col < :to` 같은 범위 조건으로 바꾸거나 함수 기반 인덱스를 검토한다. 대량 조회는 온라인 조회와 배치 집계를 분리하고, 조회 조건과 정렬 기준에 맞춰 복합 인덱스를 설계한다.
