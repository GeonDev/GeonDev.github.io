...
* **물리 트랜잭션과 논리 트랜잭션** : 1개의 트랜잭션을 사용한다는 것은 하나의 커넥션 객체를 사용한다는 것
이며 DB의 트랜젝션을 사용한다는 점에서 물리트랜젝션이라고 한다. 스프링은 성능을 위해 트랜잭션 매니저를 통해 트랜잭션을 처리하는 곳을 따로 두고 있기 때문에 이를 논리 트랜젝션이라고 분리 한다. `모든 논리 트랜잭션이 커밋되어야 물리 트랜잭션이 커밋 되고 하나의 하나의 논리 트랜잭션이라도 롤백되면 물리 트랜잭션은 롤백됨`
