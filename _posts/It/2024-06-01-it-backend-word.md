...
> - [운영체제](#용어정리--운영체제)
> - [컨테이너와 클라우드 운영](#용어정리--컨테이너와-클라우드-운영)
>   - [Docker](#docker)
...
* **운영 장애 분석 절차** : 고객센터나 운영팀에서 이슈가 들어오면 먼저 영향 범위와 심각도를 분류한다.
...
* **버그 수정과 개선 작업의 우선순위** : 운영·개발 업무에서 모든 요청을 선입선출로 처리하면 중요한 장애가 묻힐 수 있다.
...
* **Kafka 핵심 개념**
  ...
  * **ZooKeeper / KRaft** : 기존에는 ZooKeeper가 메타데이터와 리더 선출을 관리했지만, 최신 Kafka는 이 역할을 내부 쿼럼이 담당하는 **KRaft** 모드로 전환되고 있다.
