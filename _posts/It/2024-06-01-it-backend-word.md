...
* **인덱스가 동작하지 않는 경우 / 주의점** : 인덱스를 걸어도 옵티마이저가 사용하지 않거나 효율이 떨어지는 대표 케이스
  ...
  * **넓은 조회 범위와 부정 조건** : 조건에 맞는 행이 많거나 `!=`, `NOT IN`처럼 넓은 범위를 읽는다면 인덱스보다 전체 스캔이 유리할 수 있다.
...
* **영속성 컨텍스트의 이점**
  ...
  * **동일성 보장** : 같은 트랜잭션/영속성 컨텍스트 안에서는 같은 식별자를 가진 엔티티를 `==`로 비교할 수 있다.
...
* **JPA 벌크 업데이트 주의점** : JPQL 벌크 업데이트/삭제는 영속성 컨텍스트를 거치지 않고 DB에 직접 반영된다.  
  이미 1차 캐시에 올라온 엔티티 값과 DB 값이 불일치할 수 있다. 벌크 연산 후에는 `clear()`로 영속성 컨텍스트를 비우거나 별도 트랜잭션으로 분리해 오염을 막는다.

# 용어정리 – Spring

## 트랜잭션과 AOP

* **@Transactional** : Spring AOP 기반으로 동작한다. `@Transactional`이 선언되면 해당 빈에 트랜잭션 프록시가 적용되고, 메서드 호출 전후로 트랜잭션 시작/커밋/롤백을 처리한다. 기본적으로 `RuntimeException`과 `Error`가 발생하면 롤백하고, Checked Exception은 롤백하지 않는다. Checked Exception까지 롤백하려면 `rollbackFor`를 지정해야 한다.
...
* **물리 트랜잭션과 논리 트랜잭션** : DB 커넥션에 실제로 시작되는 트랜잭션을 물리 트랜잭션이라고 한다.  
  ...
  * **물리 트랜잭션** : 실제 데이터베이스에 적용되는 트랜잭션으로, 커넥션에서 커밋/롤백하는 단위
  * **논리 트랜잭션** : 스프링 트랜잭션 매니저가 처리하는 단위
...
* **Servlet Filter** : 서블릿 실행 전후에 공통 작업을 처리한다.
  ...
  * **Filter 에러 처리** : Filter 예외는 `@ControllerAdvice`가 직접 처리하지 않는다. Filter에서 직접 응답을 만들거나 error dispatch로 `ErrorController`에 전달하도록 구성한다.
...
* **Spring Security 인증 과정** : 아이디·비밀번호 인증에서는 인증 필터가 요청으로부터 `UsernamePasswordAuthenticationToken`을 만들고 `AuthenticationManager`에 전달한다. 인증 과정에서 `UserDetailsService`가 사용자 정보를 조회하고 자격 증명을 검증하며, 성공한 `Authentication`은 `SecurityContext`에 저장된다. OAuth 2.0, Bearer Token 등 인증 방식에 따라 사용하는 필터와 `Authentication` 구현체는 달라진다.
...
* **JWT(Json Web Token)** : 인증/인가에 필요한 클레임을 담고 서명한 토큰이다.  
  ...
  * **Payload(페이로드)** : 토큰에서 사용할 정보의 조각들인 클레임(Claim)이 Key/Value 형태로 담긴다. 기본적으로 암호화되지 않고 Base64URL로 인코딩될 뿐이므로 비밀번호나 개인정보 같은 민감 정보를 넣으면 안 된다.
...
* **세션 기반 인증 vs 토큰(JWT) 기반 인증**
  ...
  * **JWT 방식** : 서버의 세션 상태 의존이 작아 확장에 유리하지만, 앞서 설명한 토큰 수명·폐기 전략을 별도로 설계해야 한다.
...
* **Rate Limiting** : 특정 사용자나 클라이언트의 과도한 요청을 제한해 시스템을 보호하는 방법. API Gateway, Nginx,  
  Redis 카운터 등을 이용해 구현하며, 처리 비용이 크거나 남용 위험이 큰 엔드포인트에 특히 필요하다.


* **@Async** : Spring AOP 기반으로 동작한다. `@Async`가 선언되면 스프링이 프록시 객체를 만들어 비동기 실행을 위임한다.
...
* **CAS와 ABA 문제** : CAS는 값이 예상값과 같을 때만 변경하는 낙관적 방식이다.  
  값이 `A → B → A`로 바뀌면 CAS는 값이 처음부터 `A`였다고 오인할 수 있는데, 이를 ABA 문제라고 한다. 단순 카운터에는 보통 문제가 없지만 상태 전이가 핵심인 자료구조에서는 버전 번호를 함께 비교하는 `AtomicStampedReference` 같은 방법을 검토한다. 또한 CAS는 경합이 심하면 반복 재시도로 CPU를 소모할 수 있어, 항상 락보다 빠른 것은 아니다.
...
* **JVM** : 자바 바이트코드를 실행하는 가상 머신. 클래스 로딩, 바이트코드 실행, 메모리 관리(GC)를 담당한다.
  * **Class Loader** : VM 안으로 클래스를 로드하고 링크 작업을 수행하는 모듈
...
* **Mark & Sweep & Compact & Promotion**
  ...
  * **Compact**: Sweep 이후 생긴 빈자리를 모아 메모리 단편화를 줄이는 과정
...
* **자바의 메모리 영역**
  ...
  * **Metaspace** : 클래스 메타데이터(클래스 구조, 메서드 정보 등)가 저장되는 영역.  
    **Java 8부터 기존의 PermGen(Permanent Generation)을 대체**하며, Heap이 아닌 **네이티브 메모리(OS 메모리)에 할당**된다. 고정 크기였던 PermGen과 달리 필요하면 동적으로 크기가 늘어나 `OutOfMemoryError: PermGen space` 같은 문제를 줄였다.
...
* **HikariCP 주요 설정** : `maximumPoolSize`, `minimumIdle`, `connectionTimeout`, `idleTimeout`,  
  `maxLifetime`을 잘 잡아야 한다. DB의 wait timeout보다 `maxLifetime`을 짧게 잡아 죽은 커넥션을 재사용하지 않도록 한다.
...
* **Redis 락의 보장 범위와 장애 모델**
  ...
  * 단일 Redis 락은 장애 상황까지 포함한 강한 상호 배제를 보장하지 않는다.
...
* **락 만료와 소유권**
  ...
  * 핵심 쓰기는 DB 트랜잭션 또는 조건부 갱신으로 최종 무결성을 보장한다.
...
* **소프트웨어 아키텍처** : 시스템의 주요 구성 요소, 책임, 의존관계와 변경 원칙을 정하는 구조적 결정이다. 시스템이 커질수록 한 기능의 변경과 장애가 여러 영역으로 퍼질 수 있으므로, 아키텍처로 변경과 장애의 영향 범위를 통제한다. 좋은 아키텍처는 패턴을 많이 적용한 구조가 아니라 필요한 품질과 운영 조건을 가장 단순하게 만족하는 구조다.


* **응집도와 결합도**
  * **응집도(Cohesion)** : 하나의 모듈 안에 같은 목적과 변경 이유를 가진 기능이 모여 있는 정도다. 관련 규칙과 데이터는 가까이 두는 것이 좋다.
  * **결합도(Coupling)** : 한 모듈의 변경이 다른 모듈에 영향을 주는 정도다. 결합을 모두 제거할 수는 없으므로 안정적인 인터페이스로 의존 방향과 변경 전파를 통제한다.
...
* **Aggregate(애그리거트)** : 하나의 트랜잭션에서 일관성을 지켜야 하는 객체들의 묶음이며, 외부에서는 Aggregate Root만 상태를 변경한다. Aggregate를 너무 크게 만들면 락 범위와 로딩 비용이 커지고, 너무 작게 나누면 하나의 규칙을 여러 트랜잭션과 이벤트로 맞춰야 하므로 업무 불변식을 기준으로 경계를 정한다.
...
* **멀티 모듈 프로젝트** : 하나의 코드 저장소와 빌드 안에서 도메인·애플리케이션·인프라 또는 업무 영역별로 모듈을 분리하는 방식이다. 컴파일 의존성으로 접근 방향을 제한하고 공통 코드의 무분별한 참조를 줄일 수 있다. 단순히 모듈 수를 늘리는 것이 목적은 아니며, 순환 의존이 없고 각 모듈의 공개 API와 변경 이유가 명확해야 한다.

## MSA와 분산 시스템

* **분산 시스템(Distributed System)** : 네트워크로 연결된 여러 프로세스나 서버가 하나의 시스템처럼 협력하는 구조다. 각 노드는 독립적으로 실패할 수 있고, 네트워크 요청은 지연·유실·중복되거나 순서가 바뀔 수 있다. 원격 호출을 로컬 메서드 호출처럼 취급하면 안 되며 timeout, 재시도, 멱등성, 부분 실패와 관측 가능성을 기본 전제로 설계해야 한다.
...
* **Database per Service와 데이터 소유권** : 각 서비스는 자신의 데이터를 소유하고 다른 서비스가 해당 저장소를 직접 수정하지 못하게 하는 것이 기본 원칙이다. 반드시 물리 DB 서버를 서비스마다 하나씩 둔다는 뜻은 아니며, 스키마·계정·접근 권한 등으로 논리적 소유권을 분리할 수도 있다.
  * 다른 서비스의 데이터는 공개 API, 이벤트 또는 별도 조회 모델로 사용한다.
...
* **정합성 검증과 복구** : 재시도와 보상만으로 모든 불일치를 막을 수는 없다. 주문·결제·재고처럼 핵심 데이터는 주기적인 대사(Reconciliation) 작업으로 시스템 간 상태를 비교하고, 자동 보정이 위험한 경우 운영자가 원인과 이력을 확인해 재처리할 수 있는 관리 도구와 감사 로그를 마련한다.
...
* **Timeout과 Deadline** : 앞서 외부 연동에서 설명한 연결·응답 timeout에 더해, 분산 호출에서는 상위 요청의 전체 시간 예산인 deadline을 관리해야 한다. 하위 호출 timeout의 합이 deadline보다 길면 이미 응답할 수 없는 작업이 계속 자원을 소비하므로 남은 시간 예산을 전달하고, 취소 가능한 작업은 초과 시 중단한다.


* **Retry Storm** : 앞서 설명한 재시도를 여러 호출 계층이 동시에 수행하면 요청 수가 기하급수적으로 늘어 장애를 악화할 수 있다. 재시도 책임을 한 계층에 두고, 하위 시스템의 상태와 전체 deadline을 고려해 중단한다.
...
* **파티션 수 변경 시 주의점** : 파티션을 늘리면 병렬성은 증가하지만 Key의 파티션 매핑이 바뀔 수 있다. 순서 보장이 필요한 Topic은 파티션 수 변경이 기존·신규 이벤트의 처리 순서에 미치는 영향을 먼저 확인해야 한다.
...
* **API 응답 설계**
  * **상태 코드** : 모든 결과를 `200 OK`로 감싸지 않고, 아래에서 설명할 HTTP 상태 코드 중 요청 결과와 가장 가까운 코드를 일관되게 사용한다.
...
* **REST vs RPC/gRPC 선택 기준**
  * **REST** : 자원 중심 API, 외부 공개 API, 브라우저/HTTP 생태계와의 호환성이 필요한 경우에 적합하다.
  * **RPC/gRPC** : 행위 중심 호출, 내부 서비스 간 통신, 강한 타입 계약, 낮은 지연과 높은 처리량이 필요한 경우에 적합하다.
...
* **멱등성(Idempotent)과 안전성(Safe)** : HTTP 메서드의 성질
  ...
  * **멱등한(Idempotent) 메서드** : **같은 요청을 여러 번 보내도 결과(서버 상태)가 한 번 보낸 것과 동일한** 메서드. 네트워크 재시도 시 안전하다. (단,  
    응답 코드까지 같다는 의미는 아님)
  * **메서드별 구분** : GET과 HEAD는 안전하고 멱등하다. PUT과 DELETE는 상태를 변경하지만 멱등하며, POST와 PATCH는 일반적으로 멱등성을 보장하지 않는다. PATCH의 실제 멱등성은 변경 방식에 따라 달라질 수 있다.
...
* **구조 패턴**
  ...
  * **Proxy** : 대리 객체로 접근 제어, 지연 로딩 등을 구현한다.
...
* **스레드 안전성을 확보하는 주요 기법**
  * **공유 상태 보호** : 락이나 원자적 연산으로 충돌하는 변경을 제어한다.
  * **공유 상태 제거** : 스레드별 저장 공간이나 불변 객체를 사용해 동시에 변경할 상태를 줄인다.
  * **동시성 자료구조 사용** : 언어와 런타임이 제공하는 검증된 동시성 컬렉션을 사용한다.
  * Java에서의 `synchronized`, atomic, `ThreadLocal`, 불변 객체와 동시성 컬렉션 선택 기준은 [Java 동시성](#java-동시성)에서 설명한다.
...
* **컨테이너 vs 가상머신(VM)** : VM은 Hypervisor 위에 Guest OS 전체를 띄우는 반면,  
  컨테이너는 **호스트 OS의 커널을 공유**하고 프로세스 수준으로 격리한다. OS를 포함하지 않아 가볍고, 부팅이 빠르며, 자원 효율이 높다.
...
* **LLM(Large Language Model, 대규모 언어 모델)** : 많은 텍스트와 코드를 학습해 주어진 문맥에서 다음 토큰의 확률을 예측하는 방식으로 문장을 생성한다. 자연스러운 답변을 만들 수 있지만 사실을 데이터베이스처럼 저장·조회하는 시스템은 아니므로, 그럴듯하지만 틀린 답인 환각(Hallucination)이 발생할 수 있다.
  ...
  * **Context Window(컨텍스트 윈도우)** : 한 번의 요청에서 모델이 참고할 수 있는 입력과 출력 토큰의 최대 범위다. 범위가 크더라도 핵심 정보가 항상 동일하게 활용되는 것은 아니다.
