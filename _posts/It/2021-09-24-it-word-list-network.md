...
* **Keep-Alive와 커넥션 재사용**
  * **Keep-Alive** : HTTP 요청마다 TCP/TLS 연결을 새로 만들지 않고 기존 연결을 재사용하는 방식. 연결 수립 비용과 TLS 핸드셰이크 비용을 줄여 지연 시간을 낮춘다.
...
* **대칭키 vs 공개키(비대칭키) 암호화**
  ...
  * **활용** : 실무에서는 두 방식을 결합한다(TLS). 비대칭키/키 교환 알고리즘으로 세션키를 안전하게 합의한 뒤, 빠른 대칭키로 실제 데이터를 암호화한다.
...
왼쪽은 아래에서 설명할 프로토콜을 TCP/IP 계층별로 배치하고, 오른쪽은 자주 접하는 조합을 실제 스택 순서로 나타낸다. HTTP/1.1·2는 TLS와 TCP를, HTTP/3는 QUIC과 UDP를 사용하며, DNS는 상황에 따라 UDP와 TCP를 모두 사용할 수 있다.
