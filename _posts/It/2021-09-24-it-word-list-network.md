...
* **네트워크 전송 계층 (물리/데이터링크)**
  * **Ethernet** : TCP/IP 계층의 네트워크 접속 계층(OSI 7계층에서는 물리, 데이터링크 계층에 해당)에서 사용되는 기술입니다. IEEE 802.3 표준으로, LAN(근거리 통신망)에서 가장 널리 사용되는 유선 네트워크 기술입니다.
...
* **응용 계층**
  ...
  * **TLS/SSL** : 사용자와 웹 브라우저 간의 통신을 암호화하는 프로토콜입니다. 공개키 암호화 방식을 사용하여 안전하게 대칭키를 교환하고, 그 대칭키를 이용해 실제 데이터를 암호화합니다. 핸드셰이크 과정은 다음과 같습니다: 1) 서버가 클라이언트에게 공개키가 포함된 인증서를 전송합니다. 2) 클라이언트는 받은 인증서가 신뢰할 수 있는지 확인한 후, 무작위 데이터(pre-master secret)를 생성하여 서버의 공개키로 암호화한 뒤 전송합니다. 3) 서버는 자신의 개인키로 이 데이터를 복호화합니다. 4) 클라이언트와 서버는 이 pre-master secret을 기반으로 동일한 대칭 세션키를 생성하고, 이후의 모든 통신은 이 세션키를 사용하여 암호화합니다. 통신이 종료되면 세션키는 폐기됩니다.
  * **HTTP 3.0** : 전송 계층에 스트림을 기본 구조로 도입하는 새 인터넷 전송 프로토콜인 UDP 기반 QUIC을 사용한다. (자세한 동작은 아래 "용어정리 – 네트워크 핵심" 섹션의 HTTP 버전 항목 참고)
...
* **웹 서비스** : 웹이라는 네트워크 환경에 연결된 서로 다른 컴퓨터들이 동적으로 연결되고 소통하고 실행할 수 있도록 하는 동적환경구성을 위한 소프트웨어 컴포턴트 묶음


* **SOAP (단순객체 접근 프로토콜)** : HTTP, HTTPS SMTP등을 활용하여 XML기반의 메시지를 네트워크 상태에서 교환하는 프로토콜


* **WSDL (웹서비스 기술언어)** : 공표된 웹 서비스가 실제 어디에 위치하고 있고, 그 웹 서비스를 이용하기 위한 Biding 정보를 담고 있는 XML 마크업 언어, WSDL 정보를 해석하면 비로서 SOAP를 사용해 해당 서비스에 필요한 객체를 실행할 수 있음
...
실제 면접에서 자주 나오는 핵심 주제를 별도로 정리했다. 위 용어집과 중복되는 내용도 있지만, 면접에서 설명할 수 있도록 흐름과 비교 위주로 다시 묶었다.
...
* **HTTP**
  ...
  * **GET vs POST**
    * **GET** : 리소스 조회. 데이터를 URL 쿼리스트링에 담아 전송. 캐싱 가능, 북마크 가능, 길이 제한 있음.
    * **POST** : 리소스 생성/처리. 데이터를 본문(Body)에 담아 전송. 캐싱 안 됨, 길이 제한 사실상 없음.
...
* **HTTPS / TLS 동작**
  ...
  * **하이브리드 암호화** : 공개키(비대칭키) 방식과 대칭키 방식을 함께 쓴다.
    * 비대칭키로 안전하게 대칭키(세션키)를 교환하고,
    * 실제 데이터는 빠른 대칭키로 암호화한다. (비대칭키만 쓰면 느리고, 대칭키만 쓰면 키 전달이 위험하기 때문)
  * **TLS 핸드셰이크 흐름**
    1. 서버가 공개키가 담긴 인증서(CA 서명)를 클라이언트에 전송.
    2. 클라이언트가 인증서의 신뢰성(CA 검증)을 확인.
    3. 클라이언트가 임시 비밀값(pre-master secret)을 생성해 서버의 공개키로 암호화하여 전송.
    4. 서버가 자신의 개인키로 복호화.
    5. 양측이 동일한 대칭 세션키를 생성하고, 이후 모든 통신을 이 세션키로 암호화. 통신 종료 시 세션키는 폐기.
...
* **OSI 7계층 vs TCP/IP 4계층**
  ...
  * **TCP/IP 4계층** : 실제 인터넷에서 사용되는 실용적인 모델. OSI보다 단순하다.
    * **4. 응용 계층 (Application)** : OSI의 5~7계층(세션+표현+응용)을 포함. HTTP, FTP, DNS, SMTP
    * **3. 전송 계층 (Transport)** : OSI의 4계층과 동일. TCP, UDP
    * **2. 인터넷 계층 (Internet)** : OSI의 3계층과 동일. IP, ICMP, ARP
    * **1. 네트워크 접속 계층 (Network Access / Link)** : OSI의 1~2계층(물리+데이터링크)을 포함. Ethernet
  * **두 모델의 매핑 요약** :
    * 응용/표현/세션 (OSI 5~7) → 응용 계층 (TCP/IP)
    * 전송 (OSI 4) → 전송 계층
    * 네트워크 (OSI 3) → 인터넷 계층
    * 데이터링크/물리 (OSI 1~2) → 네트워크 접속 계층
