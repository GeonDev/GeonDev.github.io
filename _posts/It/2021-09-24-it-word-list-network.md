...
graph LR
    subgraph "HTTP 1.1 (Sequential)"
        H1A[Request 1] --> H1B[Response 1]
        H1B --> H1C[Request 2]
        H1C --> H1D[Response 2]
    end
    
    subgraph "HTTP 2.0 (Multiplexing)"
        H2A[Stream 1] -.-> H2B[Binary Frames]
        H2C[Stream 2] -.-> H2B
        H2E[Stream 3] -.-> H2B
        H2B --> H2F[Parallel Transfer]
    end
```
...
sequenceDiagram
    ...
    Note over C, S: TCP 4-Way Handshake (Close)
    C->>S: FIN
    S->>C: ACK
    S->>C: FIN
    C->>S: ACK
