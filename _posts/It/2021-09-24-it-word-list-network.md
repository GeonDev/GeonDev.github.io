...
* **커넥션 풀(Connection Pool)**
  * 외부 API나 DB처럼 반복적으로 연결하는 대상에 대해 연결을 미리 만들어두고 재사용하는 구조다.
  * 풀 크기가 너무 작으면 대기 시간이 늘고, 너무 크면 대상 서버나 NAT 포트, DB 커넥션을 고갈시킬 수 있다. 따라서 요청량, 평균 응답 시간, timeout, 대상 서버 허용 연결 수를 함께 보고 조정해야 한다.
...
![브라우저가 DNS 리졸버를 통해 루트·TLD·권한 서버에서 IP 주소를 찾고 캐시하는 과정](/images/it/network-dns-resolution.png)
...
* **X-Forwarded-For / X-Forwarded-Proto**
  ...
  * `X-Forwarded-Proto`는 원 요청이 HTTP인지 HTTPS인지 전달한다. HTTPS 리다이렉트, 보안 쿠키, 절대 URL 생성에서 중요하다.
...
* **IETF** : 개방형 절차를 통해 인터넷 프로토콜과 운영 표준을 개발하는 국제 기술 공동체다. 표준과 기술 문서는 주로 RFC 형태로 공개한다.
...
* **QoS** : 트래픽을 분류하고 우선순위, 대역폭, 지연·손실 허용 수준을 다르게 적용해 중요한 서비스의 통신 품질을 관리하는 정책과 기술의 집합이다.
