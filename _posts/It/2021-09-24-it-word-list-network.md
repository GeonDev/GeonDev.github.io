...
![OSI 7계층과 TCP/IP 4계층의 대응 관계 및 송신·수신 과정의 캡슐화 비교](/images/it/network-osi-tcpip-layers.png)


* **계층별로 나누어 보는 이유** : 앞의 요청 수명주기를 계층별로 나누면 각 단계의 역할과 프로토콜 경계를 구분할 수 있다. 장애나 지연도 DNS, TCP/TLS, HTTP, 로드밸런서, 애플리케이션처럼 구간을 분리해야 원인을 좁히기 쉽다.


* **OSI 7계층 vs TCP/IP 4계층**
  * **OSI 7계층** : 네트워크 통신 기능을 7개의 계층으로 나눈 표준 참조 모델. 실제 구현은 여러 계층에 걸칠 수 있지만, 역할과 장애 범위를 계층별로 구분하는 데 유용하다.
    ...
    * **3. 네트워크 계층 (Network)** : 논리 주소(IP) 기반 라우팅, 경로 결정. 대표 프로토콜 - IP, ICMP / 장비 - 라우터, L3 스위치. ARP는 IPv4 주소를 같은 링크의 MAC 주소로 해석하며 네트워크 계층과 데이터링크 계층의 경계에서 동작한다.
...
![연결·순서 보장·재전송을 제공하는 TCP와 빠르게 바로 보내는 UDP 비교](/images/it/network-tcp-udp-comparison.png)

* **TCP vs UDP 심화**
  * **TCP (Transmission Control Protocol)**
    * **신뢰성** : 연결이 유지되는 동안 ACK와 재전송으로 손실을 복구하며, 복구할 수 없으면 연결 오류를 알린다.
    * **순서** : 애플리케이션에 순서가 보장된 바이트 스트림을 제공한다.
  ...
  * **UDP (User Datagram Protocol)**
    ...
    * **속도** : 빠름
    * **헤더 크기** : 8바이트로 작음
    * **사용처** : 실시간 스트리밍, DNS 조회, 온라인 게임, VoIP 등 약간의 손실보다 속도/실시간성이 우선인 곳
  * **핵심 포인트** : "신뢰성이 필요하면 TCP, 속도/실시간성이 필요하면 UDP"로 기억. 단, HTTP/3는 UDP 기반 QUIC 위에서 신뢰성을 직접 구현한다.


* **TCP 3-way / 4-way handshake**

  ![TCP의 3-way 연결 수립, 데이터 전송, 4-way 연결 종료와 TIME_WAIT 흐름](/images/it/network-tcp-lifecycle.png)
  ...
  * **3-way handshake (연결 수립)** : 양방향 통신이므로 서로의 송신/수신 능력을 모두 확인해야 한다.
    ...
    * **세 단계인 이유** 양쪽의 초기 시퀀스 번호를 동기화하고, 서로의 연결 요청과 응답이 도달했음을 확인하려면 세 단계가 필요하다.
  * **4-way handshake (연결 종료)** : 연결은 양방향이므로 각 방향을 따로 닫아야 한다.
    ...
    * **네 단계인 이유** 연결 수립 때는 SYN+ACK를 한 번에 묶어 보내지만, 종료 시에는 서버가 남은 데이터를 마저 보낼 수 있어 ACK와 FIN을 분리해 보내기 때문이다.
  * **TIME_WAIT** : 마지막 ACK를 보낸 능동 종료 측(active closer)이 곧바로 연결 정보를 없애지 않고 일반적으로 2*MSL 동안 대기하는 상태다. 예시처럼 클라이언트가 먼저 종료하면 클라이언트가 이 상태에 들어가지만 서버가 능동 종료할 수도 있다.
...
* **흐름 제어(Flow Control) / 혼잡 제어(Congestion Control)**
  ...
  * **혼잡 제어 (Congestion Control)** : 네트워크(중간 라우터 등) 자체가 혼잡해지는 것을 막는 기법. (송신자 ↔ 네트워크 전체)
    * **느린 시작 (Slow Start)** : 초기 혼잡 윈도우(cwnd)에서 시작해 새 데이터를 확인하는 ACK에 따라 윈도우를 늘린다. 결과적으로 RTT마다 대략 지수적으로 증가하며, 임계값(ssthresh)에 도달하거나 혼잡을 감지하면 혼잡 회피 또는 복구 단계로 전환한다. 초기 윈도우 크기와 세부 증감 방식은 TCP 표준과 구현에 따라 달라진다.
...
![브라우저와 서버가 주고받는 HTTP 요청·응답의 시작 라인, 헤더, 본문 및 상태 코드 구분](/images/it/network-http-request-response.png)
...
* **HTTP**
  * **HTTP (HyperText Transfer Protocol)** : 웹에서 클라이언트와 서버가 데이터를 주고받기 위한 응용 계층 프로토콜.
    ...
    * **연결과 메시지의 분리** : HTTP 요청은 의미상 독립적이지만 하위 전송 연결은 재사용할 수 있다. HTTP/1.1은 지속 연결을 기본으로 사용하고, HTTP/2·3은 하나의 연결에서 여러 스트림을 처리한다.
...
![HTTP 1.1의 순차 대기, HTTP 2의 TCP 멀티플렉싱, HTTP 3의 독립적인 QUIC 스트림 비교](/images/it/network-http-versions-hol.png)
...
* **Keep-Alive와 커넥션 재사용**
  * 앞서 설명한 HTTP/1.1의 Keep-Alive는 TCP/TLS 연결 수립 비용을 줄이지만, 운영에서는 연결 양쪽과 중간 프록시의 수명 설정까지 함께 맞춰야 한다.
  * **주의점** : 서버, 로드밸런서, 클라이언트의 idle timeout 설정이 서로 맞지 않으면 이미 닫힌 연결을 재사용하다가 간헐적인 `Connection reset` 오류가 발생할 수 있다.


* **커넥션 풀(Connection Pool)**
  * 외부 API나 DB처럼 반복적으로 연결하는 대상에 연결을 미리 만들어두고 재사용하는 구조다.
  * 풀 크기가 너무 작으면 대기 시간이 늘고, 너무 크면 대상 서버나 NAT 포트, DB 커넥션을 고갈시킬 수 있다. 요청량, 평균 응답 시간, timeout, 대상 서버 허용 연결 수를 함께 보고 조정해야 한다.


* **HTTP 캐시 헤더**
  * **Cache-Control** : 브라우저나 CDN이 응답을 얼마나, 어떤 방식으로 캐시할 수 있는지 제어한다. 예를 들어 `max-age`, `no-cache`, `no-store` 등이 있다.
  * **ETag / If-None-Match** : 리소스 버전을 비교해 변경이 없으면 `304 Not Modified`로 본문 전송을 줄인다.
  * **Last-Modified / If-Modified-Since** : 마지막 수정 시간을 기준으로 캐시 유효성을 확인한다.


* **주요 HTTP 헤더**
  * **Host** : 하나의 IP에서 여러 도메인을 서비스할 때 어떤 도메인으로 요청했는지 구분한다.
  * **Content-Type** : 요청/응답 본문의 형식을 나타낸다. 예 - `application/json`
  * **Accept** : 클라이언트가 받을 수 있는 응답 형식을 서버에 전달한다.
  * **Authorization** : 인증 정보를 전달한다. 예 - Bearer token
  * **User-Agent** : 클라이언트 애플리케이션, 브라우저, 버전 등의 정보를 담는다.


## HTTPS와 TLS

![인증서를 확인하고 세션 키를 만든 뒤 암호화된 HTTP 요청과 응답을 주고받는 TLS 흐름](/images/it/network-https-tls-handshake.png)
...
* **HTTPS / TLS 동작**
  ...
  * **TLS 핸드셰이크 흐름**
    ...
    4. 클라이언트와 서버가 키 교환(ECDHE 등)으로 같은 세션키를 계산한다.
...
* **TLS 종료(TLS Termination)**
  * 클라이언트와의 HTTPS 연결을 로드밸런서나 리버스 프록시에서 복호화하고, 내부 WAS로는 HTTP 또는 다시 HTTPS로 전달하는 구조다.
  * 인증서 관리와 암복호화 부하를 프록시 계층으로 모을 수 있지만, 내부 구간 보안 요구가 높으면 프록시 이후 구간도 HTTPS로 구성한다.


* **대칭키 vs 공개키(비대칭키) 암호화**
  * **대칭키 암호화** : 암호화와 복호화에 같은 키를 사용. 속도가 빠르지만, 키를 어떻게 안전하게 상대에게 전달하느냐가 문제. AES가 대표적이며 DES는 현재 보안 용도로 사용하기에 키 길이가 짧은 레거시 알고리즘이다.
  * **공개키(비대칭키) 암호화** : 공개키와 개인키 쌍을 사용. 공개키로 암호화하면 개인키로만 복호화 가능. 키 전달 문제는 해결하지만 속도가 느리다. 예 - RSA, ECC
...
* **쿠키 보안 속성**
  * **HttpOnly** : JavaScript에서 쿠키를 읽지 못하게 해 XSS로 인한 토큰 탈취 위험을 낮춘다.
  * **Secure** : HTTPS 연결에서만 쿠키를 전송하게 한다.
  * **SameSite** : 크로스 사이트 요청에 쿠키를 보낼지 제어해 CSRF 위험을 줄인다. `Strict`, `Lax`, `None` 옵션이 있다.


## DNS와 CORS

![브라우저가 DNS 리졸버와 루트·TLD·권한 서버를 거쳐 IP 주소를 찾고 캐시하는 과정](/images/it/network-dns-resolution.png)

* **DNS 동작 과정**
  * **DNS** : 도메인 이름(예: www.example.com)을 IP 주소로 변환하는 시스템.
  * **조회 흐름** : 클라이언트는 보통 재귀 리졸버에 최종 답을 요청하고, 리졸버는 캐시에 답이 없으면 루트·TLD·권한 네임서버를 순차적으로 조회한다.
  ...
  * **주요 레코드 타입**
    ...
    * **MX** : 메일 서버 지정
    * **TXT** : 도메인 소유권 검증, SPF/DKIM 같은 메일 보안 설정 등에 사용
  * **TTL(Time To Live)** : DNS 응답을 캐시할 수 있는 시간. TTL이 길면 조회 부하가 줄지만 IP 변경 반영이 늦고, TTL이 짧으면 변경은 빠르지만 DNS 조회가 늘어난다.
...
* **CORS (Cross-Origin Resource Sharing)**
  ...
  * **Preflight 요청** : 요청 메서드·헤더·`Content-Type` 등이 CORS safelist 조건을 벗어나면 브라우저가 본 요청 전에 OPTIONS 메서드로 허용 여부를 확인한다. PUT·DELETE, `Authorization` 같은 비단순 헤더, `application/json` 요청 등이 대표적이며 서버가 허용한 경우 본 요청을 보낸다.
  * **확인 기준** : CORS는 브라우저가 동일 출처 정책을 적용하는 과정에서 발생한다. 서버 간 통신에는 브라우저의 SOP가 적용되지 않기 때문에 같은 방식의 CORS 에러가 발생하지 않는다. CORS 문제는 서버 응답 헤더와 브라우저 콘솔/네트워크 탭을 함께 확인해야 한다.
...
![클라이언트 앞의 포워드 프록시, 서버 앞의 리버스 프록시, 여러 서버로 요청을 나누는 로드밸런서 비교](/images/it/network-proxy-load-balancer.png)

* **L4/L7 로드 밸런싱 (Load Balancing)**
  * **L4 로드 밸런서** : 전송 계층(Layer 4)에서 IP와 포트 번호를 기준으로 트래픽을 분산한다. 애플리케이션 메시지를 해석하지 않아 처리 구조가 단순하지만 HTTP 경로·헤더 기반 정책은 적용할 수 없다.
...
* **X-Forwarded-For / X-Forwarded-Proto**
  * 로드밸런서나 프록시 뒤의 WAS는 직접 접속한 IP를 프록시 IP로 보게 된다. 이때 원래 클라이언트 IP를 전달하기 위해 `X-Forwarded-For` 또는 표준 `Forwarded` 헤더를 사용한다.
  * `X-Forwarded-Proto`는 원 요청이 HTTP인지 HTTPS인지 전달한다. HTTPS 리다이렉트, 보안 쿠키, 절대 URL 생성에 필요하다.
  * 단, 외부 사용자가 임의로 헤더를 조작할 수 있으므로 신뢰할 수 있는 프록시에서 들어온 헤더만 사용해야 한다.
...
* **로드밸런싱 알고리즘** : 로드밸런서가 트래픽을 여러 서버로 분배하는 기준.
  ...
  * **IP 해시 (IP Hash)** : 클라이언트 IP를 해싱해 같은 구성에서는 동일 서버로 보내는 방식으로 세션 고정에 활용한다. 서버 수나 해시 설정이 바뀌면 매핑도 달라질 수 있다.
...
![Polling, Long Polling, SSE, WebSocket의 연결 유지 방식과 데이터 전송 방향 비교](/images/it/network-realtime-communication.png)
...
* **gRPC (Google Remote Procedure Call)** : HTTP/2를 기반으로 하며 Protocol Buffers(IDL)를 주로 사용하는 RPC 프레임워크다. 바이너리 직렬화, 스트리밍, 명시적인 서비스 계약을 제공해 내부 서비스 통신에 유리하지만, 실제 성능은 메시지 크기·호출 방식·네트워크와 구현에 따라 달라지므로 REST보다 항상 빠르다고 단정할 수는 없다.
...
* **Webhook 수신 보안**
  * PG, 결제, 외부 연동 시스템은 서버가 먼저 호출하는 API뿐 아니라 상대 시스템이 우리 서버로 결과를 알려주는 webhook/callback 구조를 자주 사용한다.
  * webhook은 중복 호출, 지연 호출, 순서 뒤바뀜이 발생할 수 있으므로 이벤트 ID나 거래 ID 기준으로 멱등하게 처리해야 한다.
  * 위변조를 막기 위해 서명 검증(HMAC), 타임스탬프 검증, IP allowlist, TLS 사용을 함께 고려한다.
...
![DNS 조회부터 DB와 외부 API까지 응답 시간을 구간별로 나누어 느린 지점을 찾는 방법](/images/it/network-latency-troubleshooting.png)
...
* **네트워크 오류 메시지 구분**
  * **Connection refused** : 대상 서버의 IP/포트까지는 도달했지만 해당 포트에서 listen 중인 프로세스가 없거나 방화벽/보안 정책이 거부한 경우가 많다.
  * **Connection timed out** : 대상까지 연결 수립 응답이 오지 않은 상태다. 네트워크 경로, 방화벽, 보안 그룹, 라우팅, 대상 서버 장애를 확인한다.
  * **Connection reset by peer** : 연결이 맺어진 뒤 상대방이 비정상적으로 연결을 끊은 상황이다. 서버 프로세스 종료, idle timeout 불일치, 프록시 timeout, 애플리케이션 예외 등을 확인한다.


## 클라우드 네트워크와 보안

![사설 네트워크의 IP와 서브넷, 기본 게이트웨이 및 NAT를 통한 공인 IP 변환 과정](/images/it/network-private-ip-subnet-nat.png)

* **VPC (Virtual Private Cloud)** : 클라우드 내에 논리적으로 격리된 사용자 전용 가상 네트워크. 서브넷, 라우팅, 게이트웨이 등을 직접 구성한다.
  * **서브넷 (Subnet)** : VPC의 주소 범위를 더 작은 네트워크로 분할한 단위다. 연결된 라우팅 테이블에 따라 외부 연결 방식이 달라진다.
  * **보안 그룹 (Security Group)** : 인스턴스의 네트워크 인터페이스 등 리소스에 연결하는 상태 저장(Stateful) 가상 방화벽으로, 허용 규칙(Allow)을 정의한다.
...
* **NAT와 포트 고갈**
  * NAT는 내부 사설 IP와 포트를 외부 공인 IP와 포트로 매핑한다. 대량의 외부 API 호출이나 짧은 연결을 많이 만드는 구조에서는 사용할 수 있는 임시 포트가 부족해질 수 있다.
  * 증상은 외부 API 연결 실패, 간헐적 timeout, 특정 NAT Gateway 또는 방화벽 장비의 세션 테이블 증가로 나타날 수 있다.
  * 대응은 커넥션 재사용, timeout 조정, 불필요한 재시도 제한, NAT Gateway 분산, 대상별 호출량 제어 등이다.


* **Public Subnet / Private Subnet** : AWS에서는 라우팅 테이블에 인터넷 게이트웨이로 향하는 경로가 있으면 퍼블릭 서브넷, 없으면 프라이빗 서브넷으로 구분한다. 퍼블릭 서브넷의 리소스도 인터넷과 직접 통신하려면 공인 IPv4 또는 IPv6 주소와 허용된 보안 규칙이 필요하다. 일반적으로 인터넷 공개 ALB는 퍼블릭 서브넷, WAS/DB는 프라이빗 서브넷에 둔다.


## CDN, 서비스 메시, DNS 보안

* **CDN (Content Delivery Network)** : 지리적으로 분산된 서버망으로 사용자에게 콘텐츠를 더 빠르게 전달하는 서비스. 원본 서버(Origin)의 부하를 줄이고 지연 시간을 최소화한다.
...
![TCP/IP 네 계층에 속하는 주요 프로토콜과 HTTP·DNS의 대표 연결 조합](/images/it/network-protocol-layer-map.svg)

왼쪽은 아래에서 설명할 프로토콜을 TCP/IP 계층별로 배치하고, 오른쪽은 자주 접하는 조합을 실제 스택 순서로 나타낸다.
...
* **네트워크 전송 계층 (물리/데이터링크)**
  ...
  * **PPP (Point-to-Point Protocol)** : 두 노드 사이의 직렬 링크에서 네트워크 계층 패킷을 캡슐화하고 링크 설정·인증·오류 검출을 지원하는 데이터링크 계층 프로토콜이다. 전화선·모뎀 환경뿐 아니라 PPPoE 같은 형태로도 사용되었다.
  * **CHAP** : PPP 링크 등에서 challenge-response 방식으로 상대를 인증하는 프로토콜이다. 비밀번호 자체를 전송하지는 않지만 사용하는 해시와 운영 환경에 따라 더 강한 인증 방식이 필요할 수 있다.
...
* **인터넷 계층**
  ...
  * **IGMP (Internet Group Management Protocol)** : 호스트와 인접 라우터가 IPv4 멀티캐스트 그룹 가입/탈퇴 상태를 관리하기 위해 사용하는 프로토콜. 라우터는 IGMP로 어떤 호스트가 어떤 멀티캐스트 그룹을 수신하려는지 파악한다.
...
* **전송 계층**
  * **TCP와 UDP** : 포트 번호로 애플리케이션을 구분하는 전송 계층 프로토콜이다. 신뢰성, 연결 방식과 사용처의 차이는 앞의 [TCP와 UDP](#tcp와-udp)에서 설명한다.
...
* **응용 계층**
  ...
  * **DNS (Domain Name System)** : 이름을 주소로 해석하는 응용 계층 시스템이며, 조회 과정과 레코드 종류는 앞의 [DNS와 CORS](#dns와-cors)에서 설명한다.
  ...
  * **TLS/SSL** : 응용 계층과 전송 계층 사이에서 데이터를 보호하는 보안 프로토콜이다. SSL은 과거 명칭이며 현재 사용하는 TLS의 인증·키 교환 과정은 앞의 [HTTPS와 TLS](#https와-tls)에서 설명한다.
  * **HTTP 버전** : HTTP/1.1·2는 주로 TCP 위에서, HTTP/3는 QUIC과 UDP 위에서 동작한다. 멀티플렉싱과 HOL Blocking의 차이는 앞의 [HTTP 버전과 HOL Blocking](#http-버전과-hol-blocking)에서 설명한다.
...
* **AAA** : 인증(Authentication), 권한 부여(Authorization), 사용 기록·과금·감사(Accounting)를 묶어 접근을 통제하고 추적하는 보안 모델이다.


* **AS** : 하나의 관리 주체와 공통 라우팅 정책 아래 운영되는 IP 네트워크 집합이다. AS 사이의 경로 교환에는 BGP를 사용한다.


* **ASBR** : OSPF 영역에서 다른 AS나 라우팅 프로토콜로부터 받은 외부 경로를 OSPF 도메인에 재분배하는 경계 라우터다.
...
* **bandwidth** : 원래 신호가 차지하는 주파수 범위를 뜻하며, 네트워크에서는 링크가 이론적으로 전달할 수 있는 최대 데이터 전송률을 가리키는 의미로도 사용한다. 실제 처리량(throughput)과는 구분해야 한다.
...
* **checksum** : 송신 측이 데이터로 계산한 값을 함께 보내고 수신 측이 다시 계산해 비교하는 오류 검출 방식이다. 값이 다르면 손상을 검출할 수 있지만, 값이 같다고 모든 종류의 오류가 없음을 완전히 보장하지는 않는다.
...
* **CSU** : Channel Service Unit의 약어로, 디지털 전용 회선과 고객 장비 사이에서 회선 종단·신호 조정·진단 기능을 제공한다. 데이터 인터페이스를 담당하는 DSU와 결합해 CSU/DSU 장비로 사용되는 경우가 많다.
...
* **Domain** : DNS 계층에서 관리되는 이름 공간의 한 영역이다. 예를 들어 `example.com`은 그 아래의 호스트와 하위 도메인 이름을 관리하는 도메인이 될 수 있다.
...
* **handshake** : 본격적인 통신 전에 연결 조건이나 상태를 확인하기 위해 메시지를 교환하는 절차다. TCP 연결 수립·종료 단계와 `TIME_WAIT`은 앞의 [TCP와 UDP](#tcp와-udp)에서 설명한다.


* **HDLC** : 동기식 시리얼 링크에서 프레임을 전송하는 데이터링크 계층 프로토콜이다. 과거 Cisco 라우터의 시리얼 인터페이스에서는 Cisco HDLC가 기본 캡슐화로 사용되었다.
...
* **IETF** : 개방형 절차로 인터넷 프로토콜과 운영 표준을 개발하는 국제 기술 공동체다. 표준과 기술 문서는 주로 RFC 형태로 공개한다.
...
* **LMI** : Local Management Interface의 약어로, 프레임 릴레이에서 라우터와 스위치 사이에 PVC 상태와 연결 정보를 교환하는 관리 인터페이스다.


* **load balancing** : 트래픽이나 작업을 여러 경로·서버에 분산하는 방식이다. 계층별 차이와 분배 알고리즘은 앞의 [로드밸런서와 프록시](#로드밸런서와-프록시)에서 설명한다.
...
* **MPLS** : Multiprotocol Label Switching의 약어로, IP 주소를 매 홉마다 직접 조회하는 대신 미리 정한 레이블을 사용해 패킷 전달 경로를 구성한다. 통신사망의 트래픽 엔지니어링, VPN과 서비스 품질 제어 등에 활용된다.
...
* **NAT** : 사설 주소·포트와 공인 주소·포트를 매핑하는 네트워크 주소 변환 기능이다. 클라우드 구성과 포트 고갈 문제는 앞의 [클라우드 네트워크와 보안](#클라우드-네트워크와-보안)에서 설명한다.
...
* **ping** : ICMP Echo Request와 Echo Reply를 이용해 대상 호스트의 IP 도달 가능성과 왕복 시간을 확인하는 도구다. 응답이 없더라도 방화벽이 ICMP를 차단했을 수 있으므로 대상 서비스 전체가 장애라고 단정할 수는 없다.
...
* **QoS** : 트래픽을 분류하고 우선순위, 대역폭, 지연·손실 허용 수준을 다르게 적용해 핵심 서비스의 통신 품질을 관리하는 정책과 기술의 집합이다.


* **redistribution** : 재분배, 한 라우팅 프로토콜에서 발견한 라우팅 정보를 다른 라우팅 프로토콜의 업데이트 메시지에 분산시키는 것, 즉 RIP 정보를 IGRP쪽으로 뿌린다거나 OSPF 정보를 RIP쪽으로 뿌리는 것을 말함


* **redundancy** : 장치·서비스·연결 경로를 다중화해 일부 장애에도 서비스를 유지하는 구성이다. Active-Standby와 Active-Active 방식은 앞의 [고가용성과 세션 유지](#고가용성과-세션-유지)에서 설명한다.


* **relay** : 둘 이상의 네트워크나 네트워크 시스템을 연결하는 장치를 가리키는 OSI 용어로, 데이터 링크 계층(2계층) 릴레이는 브리지이고, 네트워크 릴레이는 라우터가 된다.


* **reliability** : 일반적으로 시스템이나 링크가 정해진 조건에서 정상 동작을 지속할 가능성을 뜻한다. 일부 네트워크 장비의 인터페이스 통계에서는 일정 기간 송수신 성공률을 바탕으로 계산한 링크 상태 지표 이름으로도 사용한다.


* **remote bridge** : 원격 브리지, WAN 링크로 물리적으로 분리된 네트워크 세그먼트를 연결하는 브리지
...
* **SOAP (Simple Object Access Protocol)** : HTTP 같은 전송 프로토콜 위에서 XML 기반 메시지를 교환하기 위한 웹 서비스 메시징 프로토콜이다.


* **WSDL (Web Services Description Language)** : 웹 서비스의 연산, 메시지 형식, 바인딩과 엔드포인트를 기술하는 XML 문서 형식이다. 클라이언트는 이를 이용해 SOAP 요청에 필요한 계약과 접근 정보를 파악할 수 있다.
