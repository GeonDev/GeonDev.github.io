...
이 테스트가 있으면 에이전트가 실수로 `SignupController`에서 `UserRepository`를 직접 주입해도 검증 단계에서 바로 드러난다. 자연어 규칙은 “왜 그렇게 해야 하는지”를 설명하고, 구조 테스트는 “정말 지켰는지”를 확인한다.
...
파일 이름은 도구마다 달라도 된다. 이름 자체보다, 에이전트가 **진입 문서에서 필요한 상세 문서와 실행 가능한 검증 명령을 쉽게 찾는 구조**가 필요하다.
