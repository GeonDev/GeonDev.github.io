...
애플에서 기존 API를 대체하는 API(App Store Connect API)를 공개하였고 보다 쉬운 마이그레이션을 위하여 애플 스토어 라이브러리를 공개하고 java, node 등 주요 언어에서 라이브러리를 사용하는 방법을 제공한다.
...
# 1. 사용 세팅
App Store Connect는 애플 스토어에 출시된 앱을 기준으로 쓰는 값이다. API를 호출할 때는 애플 개발자 콘솔에서 발급받은 키값과 어떤 앱에서 호출했는지에 대한 정보가 필요하다.
...
* 헤더 keyID에 App Store Connect에서 발급 받은 ID 사용 
* 알고리즘은 ES256을 사용
...
헤더에 명시되어 있는 대로 애플 인증서는 ES256 알고리즘으로 서명하여야 한다. 여기에 JAVA에서 인증서 파일을 디코딩 할때 인증서 파일의 앞뒤 표시(-----BEGIN PRIVATE KEY-----, -----END PRIVATE KEY-----)는 단순 표시임으로 제거 하여야 정상적인 키로 활용할 수 있다.
...
이렇게 인증서를 읽어 ECPrivateKey 객체를 생성하고 ES256 알고리즘으로 JWT 시그니처에 서명을 한다.
...
# 3. 결론
간단하게 애플 스토어 라이브러리를 사용하지 않고 API를 사용하는 방법을 알아 보았다. 코드를 보면 알겠지만 모델을 따로 만들어야 하고 검증 로직을 별도로 만들어야 하는 등 많은 불편함이 있어 개발환경이 JDK11 이상 이라면 라이브러리 사용을 권장한다.
