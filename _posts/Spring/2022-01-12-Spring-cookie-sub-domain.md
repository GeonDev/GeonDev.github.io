...
Author: Geon Son
categories: Spring
tags: [Cookie, Subdomain, Servlet, Session]
comments: true
toc: true    
---
...
쿠키의 도메인은 최상위 도메인(`.com`)이나 공개 접미사(public suffix)로는 지정할 수 없을 뿐, `example.com`처럼 등록 가능한 도메인이면 된다. (점 개수로 판단하는 것이 아니라, TLD/공개 접미사인지가 기준이다.)   
