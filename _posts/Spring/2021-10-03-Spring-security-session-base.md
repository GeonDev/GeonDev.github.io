...
기존에 시큐리티를 쓰지 않을 때는 암호화를 위해 별도의 SHA-256 함수를 작성하고, 로그인 관리를 위해 세션에 직접 정보를 저장해야 했다. 시큐리티를 적용하면 이런 과정을 맡아 주고, 옵션에 따라 접속 권한도 다양하게 조절할 수 있다.
      ...
      client:
        registration:
          google: # /oauth2/authorization/google 이 주소를 동작하게 한다.
            client-id: ENC(YOUR_ENCRYPTED_CLIENT_ID)
            client-secret: ENC(YOUR_ENCRYPTED_CLIENT_SECRET)
            scope:
            - email
            - profile
```
