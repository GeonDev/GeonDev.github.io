...
- **검증 목표**: 이 테스트는 `purchaseProcessService`의 `process` 메서드가 호출되었을 때, **재고가 충분한 상품에 대한 구매가 성공적으로 이루어지는지**를 검증한다. 구체적으로는 다음을 확인한다:
    - 상품의 재고가 구매 수량만큼 **정확히 감소하는지**.
    - 총 구매 금액이 **올바르게 계산되는지**.
    - `productRepository`, `purchaseRepository`, `purchaseProductRepository`의 관련 메서드들이 **예상대로 호출되는지**.


- **테스트 코드**: 앞서 "Given-When-Then (GWT) 패턴" 절에서 본 `process_should_decreaseStockAndSucceed_when_productInStock_gwt()` 와 동일하다. 위 `setUp()`에서 준비한 `testUser`·`testProduct`를 그대로 사용하며, 재고 감소·총액 계산·각 Repository 호출 여부를 함께 검증한다.

## 좋은 단위 테스트 작성 패턴: 네이밍과 경계값 테스트
### 테스트 케이스 네이밍 규칙
- **추천 네이밍 형식: `메서드명_should_기대행위_when_조건`**
    - **`[메서드명]`**: 테스트하려는 대상 메서드의 이름이다. (예: `process`)
    - **`should_[기대행위]`**: 해당 조건에서 메서드가 어떻게 동작해야 하는지 서술한다. (예: `thenStockIsDecreased`, `thenThrowsNoSuchElementException`)
    - **`when_[조건]`**: 테스트가 진행되는 특정 조건이나 상황을 서술한다. (예: `givenProductInStock`, `givenNonExistentProduct`)
...
견고한 코드를 만들기 위해서는 이러한 엣지 케이스(Edge Case)를 집요하게 테스트해야 한다.
...
1. 기대한 예외가 실제로 발생하는가
2. 예외가 발생했을 때, **일어나면 안 되는 동작(예: 저장)이 실행되지 않았는가** (`verify(..., never())`)

```java
  @Test
  @DisplayName("존재하지 않는 상품을 구매하면 예외가 발생하고 구매 저장이 일어나지 않는다")
...
# 최신 테스트 기능 (Spring Boot 3.4+ / JUnit 5)

앞의 예제들은 `@SpringBootTest`로 전체 컨텍스트를 띄우는 방식이 많았는데, 최근 버전에서는 더 가볍고 표현력 좋은 도구들이 추가되었다. 정리해 두면 다음과 같다.
...
스프링 컨텍스트 안의 Bean을 Mock으로 교체할 때 쓰던 **`@MockBean` / `@SpyBean`은 Spring Boot 3.4(Spring Framework 6.2)부터 deprecated** 되었다.
...
## 4. Testcontainers - 진짜 DB로 통합 테스트
Mock이나 H2로는 잡히지 않는 문제(실제 DB의 방언, 인덱스, 동시성 등)가 있다.
**Testcontainers** 는 테스트 실행 시점에 도커로 실제 DB(MySQL, PostgreSQL 등) 컨테이너를 띄워주고, 끝나면 정리한다.
