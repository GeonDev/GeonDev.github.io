...
Author: Geon Son
categories: Spring
tags: [Cookie, SameSite, CORS, Spring Boot, Tomcat]
comments: true
toc: true    
---

크롬이 업데이트 되면서 쿠키 공유 옵션을 엄격하게 결정해주고 있다. 사실 이 내용은 21년 부터 고지되고 시행되었지만  
...
CORS 경고는 간단하게 서버에 허용할 도메인을 넣음으로 해결할수 있다. 백엔드 파트에서 적용하고 프론트에서 확인하면 끝.  
...
이 때문인지 프론트 페이지와 통신을 할때 가끔씩 문제가 발생했고 크롬에서만 문제가 발생한 다는 것과 원인이 sameSite라는 걸 알게 되었다.
...
sameSite 설정만으로 충분해 보이지만 secure 설정과 도메인 지정이 필요한 경우가 있다. 크롬의 정책 때문이다.
**same-site 가 none이라면 https 호출만 허용한다.** 그렇기 때문에 통신을 할때 secure: true 옵션을 추가하여 쿠키를 https로 통신할때만 보낼거라는 선언을 한다.  
이렇게 선언을 하고 난 이후에 프론트에서 쿠키의 정보를 읽을수 있어야 하기 때문에 같은 서브도메인을 사용할거라는 선언으로  
도메인을 넣어주면 프론트엔드에서 쿠키를 읽을수 있다.

참고로 sameSite 옵션은 다음과 같은 것들이 있다.
...
lax에서 허용하는 경우는 `<a>` 태그나 다른 링크를 통하여 다이렉트로 페이지 이동을 하는 경우에 허용을 한다.  
...
## 2.2. 톰캣 설정
어플리케이션에서 설정하는 방법도 있고 톰캣과 웹서버에서도 설정할 수 있다. 웹서버 관리 권한이 없어서 톰캣 설정으로 처리했다.
톰캣 설정도 간단하다. 그냥 server.xml에 설정값 넣고 재부팅 시키면 된다.
