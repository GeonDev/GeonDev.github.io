...
Author: Geon Son
categories: Spring
tags: [Spring MVC, JPA, Generics, Abstraction]
comments: true
toc: true
---
...
Controller는 공통적으로 하나 이상의 Service를 사용하게 되므로 Controller가 사용하는 Service 역시 특정한 함수를 포함하고 있다는 것을 보장 해야 한다.

![](/images/it/e632e6cfimage1.png){: .align-center}


이 기능을 구현할 때는 추상 클래스나 인터페이스를 쓰면 된다.
처음에는 이렇게 코드를 작성하더라도 결국에 개별 Service에서 실제 사용될 기능을 구현하는 과정이 필요하기 때문에 굳이 이럴 필요가 있을까? 하는 생각과
...
# 2. 구현
내가 실제 구현을 할때는 controller 부터 반대로 진행하면서 추상화를 했지만 설명할때는 진행 순서대로 작성하려고 한다.
...
JpaRepository를 @Autowired 하기 위해서는 스프링 빈이라는 것을 알려야 하므로 @Component를 선언한 것 정도가 특별한(?) 점 이다.
...
간단하게 만들었기 때문에 특별한 기능은 없다. CRUD 메서드가 반드시 있고, `JpaRepository`가 반드시 있다는 점 정도만 보면 된다.

## 2.2 Base Controller 생성
