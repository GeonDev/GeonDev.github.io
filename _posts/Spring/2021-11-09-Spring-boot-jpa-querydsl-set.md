...
```
우선 Querydsl을 사용하기 위해 라이브러리를 추가한다.  
이 글의 예제는 **Spring Boot 2.x + Maven** 기준으로 작성되었기 때문에, 사용 중인 Spring Boot / Querydsl 버전에 따라 의존성 버전이나 annotation processing 설정은 달라질 수 있다.
...
QueryDsl에서도 Spring Data JPA의 Pageable을 사용할 수 있다. 지난번에 이미 QuerydslRepositorySupport를 구현하였기 때문에 그대로 활용하는 방법으로 구현하였다.
...
화면에 표시하기 위해 만든 Pagination 객체는 대략 이정도이다.
