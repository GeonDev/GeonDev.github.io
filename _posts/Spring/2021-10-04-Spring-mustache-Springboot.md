...
회사에서 진행 중인 프로젝트, 진행된 프로젝트는 대부분 Spring(boot) + jsp로 작업되어 있었다. 작업한 프로젝트 대부분이 운영(Admin) 페이지가 많기 때문에 접속량이 많은 편도 아니고 프론트를 전담하는 개발자가 없기 때문에 이전 개발자 분들에게 익숙한 방식으로 개발되었고, 같은 방식으로 계속 개발되고 있다고 생각한다.
...
mustache는 스프링부트에서 공식 지원하므로 이 설정만 추가하면 적용된다.



#  화면에서 연산 불가
mustache의 가장 큰 특징은 서버에서 완벽하게 연산된(?) 데이터만 전달 해야 한다는 점이다.
화면에 복잡한 연산 코드가 없어 구조가 깔끔하고 읽기 쉬운 화면을 구성한다.
JSP로 화면을 구성하다 보면 `c:set`, `c:forEach`처럼 화면단 연산 코드가 많아져 복잡해진다. Mustache는 이런 코드 없이 서버단에서 필요한 정보를 넘겨주기 때문에 비교적 깔끔하다.
...
서버의 서비스 단에서는 지금이 첫 페이지인지 마지막인지, 다음 번호는 몇 번인지, 표시할 번호는 무엇인지 같은 값을 계산해야 한다. `JPA Pageable`은 0부터 페이지를 세지만 화면에는 1부터 보여줘야 해서 이런 작은 연산도 서버에서 처리해야 한다.
...
이 부분은 단점이자 장점인데 서버에서 화면으로 전달하는 값은 반드시 있는 값 이라는 큰 원칙을 잡고 화면을 구성하기 때문에 화면 자체만 보면 오히려 간단하게 작성된다는 특징이 있다.
