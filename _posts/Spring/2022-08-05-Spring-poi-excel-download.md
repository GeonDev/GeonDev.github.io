...
엑셀파일을 다루기 위하여 사용하는 클래스이다. (XSSFWorkbook은 읽기/쓰기가 모두 가능하지만, SXSSFWorkbook은 대용량 파일을 스트리밍으로 **쓰기 전용**으로 생성할 때 사용한다. 기존 파일을 읽을 때는 XSSFWorkbook을 쓴다.)
...
public Workbook generateUserExcel(Map<String, Object> model) throws Exception{
	List<User> userlist = (List<User>) model.get("list");
		...
		SXSSFWorkbook workbook = new SXSSFWorkbook(xwb);
			...
			logger.info("template excel READ Complete");
	        ...
	        // Set the address of the top left hand cell.
...
라는 오류가 발생했다. 이미 다른 기록이 작성되어 있다는데 엑셀파일은 빈칸이다. 크게 2가지 해결방법이 있다.
