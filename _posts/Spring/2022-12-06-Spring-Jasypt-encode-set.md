...
Author: Geon Son
categories: Spring
tags: [Jasypt, Encryption, Spring Boot]
comments: true
toc: true    
---
...
서버 설정으로 지정하거나 환경 변수로 설정하는게 좀더 안전하기는 하다. 다만 담당자가 변경되다가 분실되는 경우가 있어 코드에 추가(권장하지는 않음)
...
datasource:
  ...
  username: ENC(암호화된_username_문자열)
  password: ENC(암호화된_password_문자열)
```
