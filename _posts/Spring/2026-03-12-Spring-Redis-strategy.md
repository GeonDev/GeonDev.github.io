...
- 직렬화에 성공하더라도, 역직렬화된 엔티티는 영속성 컨텍스트와 무관한 준영속(detached) 객체
...
직렬화 문제는 설정을 잘 맞추면 피해갈 수도 있지만, 마지막 항목인 준영속 문제는 구조적이다. 캐시에서 꺼낸 엔티티는 겉보기에 멀쩡해서 호출부가 영속 상태로 착각하기 쉬운데, lazy 필드에 접근하면 `LazyInitializationException`이 나고, 수정해서 `save()`에 넘기면 merge가 일어나 의도치 않은 UPDATE가 나갈 수 있다. 트래픽 장애 튜닝에서 게시판 검증 조회에 캐시를 붙일 때도 엔티티 대신 판정에 필요한 값만 담은 전용 DTO를 새로 만들어 캐시했다.

이때 모든 호출부를 DTO로 교체할 필요는 없었다. 트래픽이 몰리는 **읽기 경로만** DTO 캐시로 전환하고, 엔티티가 실제로 필요한 쓰기 경로는 캐시 없이 기존 DB 조회를 유지했다. 캐시의 효과는 대부분 읽기 경로에서 나오므로, 준영속 엔티티를 쓰기 로직에 흘려보내는 위험을 감수하면서까지 전면 교체할 이유가 없다.
...
**"짧게 유지"와 stampede 방지는 지터의 방향(음수)만 잡으면 상충하지 않는다.** 음수 지터로
