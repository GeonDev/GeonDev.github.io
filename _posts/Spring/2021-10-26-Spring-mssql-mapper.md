...
스카우터에서 특정 쿼리를 수행할 때 속도가 너무 느리다는 알람이 계속 출력되었다. 출력하는 양이 많아서 그런지 테스트를 해보았는데 조회되는 수량이 없다. 스캔 수가 많을 수는 있지만 쿼리 결과가 0인 쿼리가 타임아웃이 걸릴 정도로 느리다는 점이 이상했다. Mybatis의 쿼리를 DB 도구로 가져와 실행하면 0.01초 만에 끝났다. 애플리케이션과 DB 도구의 실행 조건이 달랐다.
