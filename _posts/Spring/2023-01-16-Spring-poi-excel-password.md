...
Author: Geon Son
categories: Spring
tags: [Apache POI, Excel, Encryption, Spring Boot]
comments: true
toc: true    
---

엑셀파일을 다운로드 하는데 가끔 암호화를 요구 하기도 한다. 
처음에는 어떻게 만들어야 하는지 몰랐는데 의외로 간단하게 엑셀파일 생성 부분만 수정하면 된다.  
이전에 컨트롤러 등은 구현한 적이 있으므로 이전 포스팅을 참고 하고 암호화 부분만 살펴 보려고 한다.
...
마지막으로 OPCPackage에 위에서 작성했던 EncryptionInfo로 인코딩된 데이터를 넣어주면 된다.
...
POI 4.0 이상으로 라이브러리를 사용하면 이상하게 암호화 해제를 할때 파일이 깨지는데 추후에 보충이 필요할 것 같다.