...
Author: Geon Son
categories: Spring
tags: [Dependency Injection, ApplicationContext, Spring Bean, IoC]
comments: true
toc: true    
---
...
스프링 프레임워크에서 관리하지 않는 클래스에 Service를 사용하고 싶었다. 내가 생성한 쓰레드에서 Service 클래스를 이용해서 DB에 데이터를 삽입하고 싶었다. 아무 생각없이 쓰레드에 @Autowired를 하면 당연히 NullPointerException을 날린다.
...
ApplicationContextProvider 클래스는 IoC를 수행하는 ApplicationContext를 넘겨주는 역할을 수행한다.
...
클래스를 생성하는 과정에서 Controller나 Service에서 이미 주입이 끝난 Class를 주입해 주었다.
