...
이런 식으로 객체 전체보다 특정 칼럼 값을 불러오는 경우가 더 많아서, 클래스 변수들도 그에 맞게 모두 수정했습니다.
...
사실 문제는 이렇게 만들어진 엔티티가 단방향이었기 때문에, 다른 엔티티 값을 참조하는 로직이 많아질수록 조회 쿼리와 객체 탐색 방식이 더 복잡해졌다는 점이었습니다. 그래서 단방향 매핑을 양방향으로 전환하면서 고려해야 하는 내용을 테스트하고 정리하였습니다.
