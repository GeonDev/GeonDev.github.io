...
Author: Geon Son
categories: Spring
tags: [Spring MVC, Jackson, JSON, ViewResolver, Spring Boot]
comments: true
toc: true
---
...
이 과정의 설정을 줄이는 `JsonView` 라이브러리를 회사에서 사용했고, 그 사용법을 정리한다.
...
두 라이브러리를 추가해도 동작하지만, 스프링부트에서는 별도 라이브러리 없이 설정할 수 있다.
```xml
		<dependency>
...
일반적인 컨트롤러와 비슷하게 작성하면 된다. 기존에는 Map이나 List를 새로 만들어 데이터를 전달했는데, 이제는 그런 작업 없이 `ModelAndView`나 `String`을 사용해 view resolver를 쓰듯 전달하면 파싱된 데이터가 넘어간다.
