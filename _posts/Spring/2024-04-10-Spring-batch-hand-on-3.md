...
Author: Geon Son
categories: Spring
tags: [Spring Batch, ItemReader, Chunk]
comments: true
toc: true    
---
...
이제 생성한 CustomItemReader를 통하여 Person 객체를 전달 받고 출력하는 소스코드를 작성한다.  
...
~~~java
    private FlatFileItemReader<Person> csvFileItemReader() throws Exception {
        ...
        /** CSV파일을 Person 객체에 매핑 **/

        //파일을 한줄씩 읽을수 있는 LineMapper 설정
        DefaultLineMapper<Person> lineMapper = new DefaultLineMapper<>();

        //파일의 필드명을 설정할수 있는 Tokenizer 선언 
        ...
        FlatFileItemReader<Person> itemReader = new FlatFileItemReaderBuilder<Person>()
                ...
                .encoding("UTF-8")
                // Resource 디렉토리 아래 파일을 읽을수 있는 스프링 클래스
                .resource(new ClassPathResource("test.csv"))
                //첫번째 라인을 무시 (Person의 필드명이기 때문에)
                .linesToSkip(1)
...
JdbcCursorItemReader는 DB기반 읽기 이므로 예제를 위한 sql 코드를 생성한다.  
생성한 코드는 resource 폴더 아래에 person.sql로 지정하였다.
~~~sql
create table person (
...
* 스프링 배치(Spring Batch) 4.3+ 에서 JPA 기반 Cursor ItemReader(JpaCursorItemReader)가 제공됨
...
jdbc와 마찬가지로 jpa를 사용하기 위해서 EntityManagerFactory를 받아와야 한다.  
