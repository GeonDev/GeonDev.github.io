...
연결이 가장 많은 노드 상위 10개(`BacktestRequest` 136개 엣지, `BacktestService` 124개, `StockPriceDto` 123개 등)를 "조심할 곳"이 아니라 "복잡한 곳부터 순서대로 읽을 목록"으로 썼다. 이 목록 기준으로 `BacktestService`, `SimulationEngine`, `TechnicalIndicatorService`를 먼저 읽었고, 위 두 사례도 그 과정에서 나왔다.

## 중복처럼 보이는 신호도 확인이 필요하다

`GRAPH_REPORT.md`는 "Springdoc Prod-Disable Convention Repeated Across All Services"라는 하이퍼엣지로 7개 서비스의 `application.yml`에 같은 설정이 반복된다고 잡았고, `CLAUDE.md`↔`AGENTS.md`, `README.md`↔`INSTRUCTIONS.md`도 Surprising Connection으로 묶었다. 원래 목적("중복 찾기")에 들어맞아 보였지만, 확인해보니 둘 다 의도된 구조였다.

- Springdoc 설정은 `docs/rules/SECURITY_INFRA_GUIDE.md`에 문서화된 공통 정책(`${SPRINGDOC_API_DOCS_ENABLED:false}`)을 각 서비스가 그대로 따른 것이었다.
- 문서 쌍은 `.claude/rules/`·`.codex/rules/`가 `docs/rules/`를 가리키는 심볼릭 링크였고, AI 도구별 진입 문서만 분리해둔 설계였다.

두 건 모두 실제 조치로는 이어지지 않았다. 좋아 보이는 신호도 그대로 믿지 않고 확인하는 과정이 필요했다는 뜻이다.

# 한계 1. 커뮤니티 라벨 중복은 라벨링 스크립트의 한계다
...
# 한계 4. AST 정적 분석은 런타임에 자동으로 연결되는 관계를 놓친다
...
`graphify-out/`에는 `graph.json`·`GRAPH_REPORT.md`·`graph.html` 외에 `manifest.json`, `cache/`, `cost.json`처럼 매 실행마다 남는 부산물이 쌓인다. 무엇을 지키고 무엇을 지워도 되는지는 용도에 따라 갈린다.

## 캐시와 매니페스트는 지우면 안 된다

`manifest.json`은 파일별 해시를 기록해 `--update`가 변경된 파일만 골라내는 근거고, `cache/`는 문서·이미지의 시맨틱 추출 결과를 프롬프트 버전과 함께 저장해 같은 파일을 다시 LLM에 태우지 않게 한다. 이 둘을 지우면 다음 `--update`가 코퍼스 전체를 처음부터 다시 추출한다 — 한계 2에서 다룬 토큰 비용이 매번 다시 발생한다는 뜻이다. `graph.html`이나 `GRAPH_REPORT.md`는 언제든 재생성되므로 지워도 되지만, `manifest.json`과 `cache/`는 재추출 비용을 결정하는 핵심 상태다.

## 커밋 여부는 크기와 협업 방식에 따라 정한다
...
`graphify hook install`은 커밋 후 자동으로 변경된 코드 파일만 재추출해 `graph.json`과 `GRAPH_REPORT.md`를 갱신하는 post-commit 훅을 심어준다. 문서·이미지 변경은 훅이 잡지 않으므로 `--update`를 따로 돌려야 한다. 한계 3에서 다룬 "중단된 세션이 임시 파일을 남기는" 문제는 이 훅으로 자동화하든 수동으로 돌리든 똑같이 남는 위험이라, `graphify-out/`에 오래된 `.graphify_chunk_*.json`이 있는지는 그래프를 신뢰하기 전에 한 번씩 확인하는 편이 낫다.
