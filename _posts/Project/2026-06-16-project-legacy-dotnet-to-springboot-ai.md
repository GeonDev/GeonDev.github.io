---
layout: post
title: 레거시 .NET을 Spring Boot로 전환하기 위한 AI Rules & Skills 적용기
date: 2026-06-16
Author: Geon Son
categories: Project
tags: [Legacy Migration, ASP.NET, Spring Boot, AI, Stored Procedure]
comments: true
toc: true
---

> 사내 레거시 시스템(ASP.NET WebForms)을 Spring Boot로 전환하는 프로젝트를 진행하면서,
> AI 코딩 에이전트(Claude Code)를 **그냥 쓰지 않고 "규칙(Rules)과 스킬(Skills)" 을** 사용한 경험을 정리한다.
...
물어볼 사람도, 참고할 문서도 없었다. **유일하게 신뢰할 수 있는 명세는 "지금 돌아가는 레거시 소스와 SP" 그 자체**였고,
이 사실이 뒤에 나오는 모든 원칙 — "추측하지 말고 원본을 확인하라", "기능을 문서로 남겨라" — 의 출발점이 됐다.
...
4. **죽은 기능까지 전환** — 레거시 `.aspx`에 `<!-- ... -->`로 막아둔 버튼을 멀쩡한 기능으로 되살린다.
...
3. **규칙의 책임이 명확해짐** — 한 파일 = 한 관심사(네이밍, 에러 처리, 테스트 …). 규칙을 고칠 때 수정 위치가 바로 보이고, 사용되지 않는 규칙이 줄었다.
...
Andrej Karpathy가 언급한 것으로 알려진 LLM 코딩 함정을 행동 지침으로 옮긴 스킬이다. 요지는 네 가지.
...
핵심은 **결과셋 인덱스(`getList(0, ...)`, `getInt(1, ...)`)를 레거시 `ds.Tables[n]` 순서와 똑같이 맞추는 것** — 이걸 추측하지 않고 원본에서 확인하도록 절차로 강제한 게 안정성의 8할이었다.
