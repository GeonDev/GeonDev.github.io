...
쿼리 자체는 빠르게 끝났지만 `RESULT-SET-FETCH` 구간에서 시간이 늘어났다. DB에서 결과를 받은 뒤 애플리케이션 객체로 매핑하는 과정까지 포함된 시간으로 추정했다. 이 글에서 확인한 원인은 투표 이력 조회에 필요하지 않은 컬럼과 엔티티까지 함께 읽는 구조였다.


~~~
`RESULT-SET-FETCH` 시간을 줄이기 위해 조회 범위를 먼저 줄였다.

인덱스나 커넥션 풀 크기 변경은 이 측정의 직접적인 대상이 아니었다. 해당 문제에서는 투표 이력 확인에 필요한 값만 DTO로 조회하고, 네이티브 쿼리로 변경하는 방법을 적용했다.
~~~
