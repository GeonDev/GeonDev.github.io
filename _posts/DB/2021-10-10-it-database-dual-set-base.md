...
오라클에서 발생한 모든 데이터의 변경 사항을 기록하는 파일. 서버 프로세스가 변경 내용을 리두 로그 버퍼(캐시)에 기록하고, LGWR(Log Writer)가 그 버퍼 내용을 리두 로그 파일에 기록한다. (LGWR는 버퍼에 쓰는 것이 아니라 버퍼를 파일로 내보내는 역할이며, 커밋·버퍼 임계치 도달 등의 시점에 동작한다.)
...
주요 솔루션은 Oracle의 Golden Gate, Quest의 Share Flex가 있고, 오픈소스 CDC 도구로는 Debezium 등이 있다. (참고로 Galera는 CDC가 아니라 MySQL/MariaDB용 동기 다중 마스터 복제 솔루션이다.)
